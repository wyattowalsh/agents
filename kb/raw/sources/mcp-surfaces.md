...
`scrapling` is tracked as a stdio MCP from `D4Vinci/Scrapling` launched via `scripts/mcphub/scrapling-stdio.sh` → `uvx --from scrapling[ai]==0.4.10 scrapling mcp`. Tools include HTTP fetch (`get`, `bulk_get`), browser fetch (`fetch`, `stealthy_fetch`, sessions, `screenshot`). The server is opt-in through MCPHub groups (`research`, `media-work`, `live-browser`, `heavy`, `experimental` full; bounded `get`/`bulk_get` in `web-read`; bounded `get` in `shared-read`) and is excluded from default `harness`, `tunnel`, and capability `browser` (Chrome DevTools). Browser tools require maintainer `scrapling install`; proxy and `SCRAPLING_EXECUTABLE_PATH` overrides belong in user-owned `.env.mcphub` only. Scraped content is untrusted evidence.
