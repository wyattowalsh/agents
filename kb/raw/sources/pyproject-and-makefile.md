---
title: Pyproject And Justfile Source Note
tags:
  - kb
  - source
aliases:
...
# Pyproject And Justfile Source Note
...
| original_location | `pyproject.toml`; `justfile` |
...
`pyproject.toml` defines the `wagents` Python project, Python `>=3.13`, Typer/Rich/HTTP dependencies, dev tooling, pytest paths, Ruff settings, Ty configuration, and an empty `uv` workspace member list. `justfile` exposes install, validation, test, lint, typecheck, audit, package, OpenSpec, MCPHub, and README regeneration recipes.
...
| Just recipes wrap `uv run wagents validate`, `uv run pytest`, `uv run ruff check`, `uv run ty check`, MCPHub scripts, and OpenSpec commands. | `justfile` | canonical material | Development section. |
