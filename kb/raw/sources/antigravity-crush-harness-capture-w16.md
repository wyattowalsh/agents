...
| original_location | Root `GEMINI.md`; `justfile` install recipes; `README.md` supported agents |
| capture_method | repo bridge + Skills CLI adapter pointers |
| captured_at | 2026-06-25 |

## Summary

**Antigravity** and **Gemini CLI** share the `GEMINI.md` → `@./AGENTS.md` → `@instructions/global.md` bridge. **Crush** reads `AGENTS.md` → `@instructions/global.md` per `AGENTS.md` supported-agents table. Skills install: `just install-antigravity` / `just install-gemini` / `just install-crush` (Skills CLI `-a antigravity`, `gemini-cli`, `crush`).
