...
Gemini CLI is the official Google terminal agent with MCP support, `GEMINI.md` context files, and non-interactive mode. This repo bridges via root `GEMINI.md` → `@./AGENTS.md` → `@instructions/global.md`, sharing the Antigravity instruction path. Skills install target: `just install-gemini` / `npx skills add ... -a gemini-cli`.
