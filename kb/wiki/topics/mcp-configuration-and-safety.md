...
Registry-managed MCPs with distinct safety framing include: `ossinsight` (public API rate limits), `supathings` (local macOS Things 3 read/write), `open-websearch` (scraping-based multi-engine search and page fetch without API keys), `scrapling` (CSS-targeted web scraping with optional browser spawn and anti-bot fetch), `reddit-mcp-buddy` (read-only Reddit browse/search/user tools), and `jupyter-mcp-server` (live Jupyter notebook and kernel control against user-owned JupyterLab). `open-websearch` is opt-in via MCPHub groups only; the repo wrapper sets `MODE=stdio` and `SEARCH_MODE=request` to avoid starting the upstream HTTP daemon or Playwright by default. `scrapling` is opt-in via MCPHub groups only; browser tools require maintainer `scrapling install`; upstream `main_content_only` sanitization reduces prompt-injection risk but is not repo-enforced. Treat fetched pages as untrusted; keep proxy, `FETCH_WEB_INSECURE_TLS`, and `SCRAPLING_EXECUTABLE_PATH` overrides in user-owned environment only. Close persistent `scrapling` browser sessions with `close_session`. `reddit-mcp-buddy` is opt-in via MCPHub groups only (`research`, `shared-read`, `experimental`); anonymous mode works without credentials; optional Reddit OAuth secrets stay in local `.env.mcphub` only (prefer app-only client id/secret over password grant); treat Reddit content as untrusted evidence. `jupyter-mcp-server` is opt-in via MCPHub groups only; `execute_cell` and `execute_code` run arbitrary kernel code; keep `JUPYTER_TOKEN` in `.env.mcphub` only; avoid `connect_to_jupyter` in production; treat notebook cell sources and outputs as untrusted evidence.
...
| Surface | Safety rule |
|---------|-------------|
| Tracked settings | `mcp/mcphub/mcp_settings.json` must stay secret-free; `just mcphub-validate` enforces registry parity and bearer-routing invariants |
...
| MCPHub settings validation enforces no tracked secrets and routing/auth invariants. | `kb/raw/captures/mcphub-settings-validation-capture-w07.md`; `scripts/mcphub/validate_settings.py` | raw capture and canonical script | Current recipe is `just mcphub-validate`; validation returned `ok` on 2026-06-25. |
