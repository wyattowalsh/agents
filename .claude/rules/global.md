...
**Grok trivial offload (Tier-T)**: When fast preflight `ok` and OAuth is healthy, parents MAY offload a **bounded leaf** via `/grok-delegate trivial` (≤3 file reads OR ≤1 file edit ≤80 LOC; no destructive/prod/git-push). Parent retains synthesis. Skip Tier-T for multi-node graphs or overlapping writers.
