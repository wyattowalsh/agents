...
OpenCode has a growing plugin inventory split across the repo-managed project mirror and the live user config. The requested additions include runtime plugins, TUI-only plugins, scheduler behavior, Claude credential reuse, and Langfuse telemetry. Without a tracked change, future sync or validation work can lose the distinction between runtime and TUI surfaces, accidentally commit secrets, or introduce unsupported plugin specs.
...
- Update OpenCode docs and global instructions with guardrails for scheduler jobs, Claude auth, Langfuse secrets, and UI/workflow plugin deferrals.
