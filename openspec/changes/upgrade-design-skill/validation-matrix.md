...
| Chrome DevTools skills externalized | `for s in chrome-devtools chrome-devtools-a11y-debugging chrome-devtools-cli chrome-devtools-debug-optimize-lcp chrome-devtools-memory-leak-debugging chrome-devtools-troubleshooting; do test ! -d "skills/$s"; done` | Operational Chrome DevTools workflows are curated-external rows, not repo-owned skill directories. |
...
| Folded catalog rows | `jq` checks against `docs/public/generated-registries/skills-catalog-index.json` | Exactly one custom `design` row, six curated-external Chrome DevTools rows, zero active custom `chrome-devtools*` rows, and zero active folded non-Chrome custom/external rows. |
...
- Generated catalog JSON contains curated-external rows for the official
  Chrome DevTools operational skills and no active custom `chrome-devtools*`
  rows.
...
- Remaining folded-skill text hits are categorized as official external
  Chrome DevTools handoffs, historical evidence, research synthesis, or fixed.
