...
Mapping security ownership in codebases/projects. From official openai/skills catalog for Codex (and compatible). Curated or experimental skills in .curated/.experimental.
...
Install: `npx skills add openai/skills --skill security-ownership-map -y -g -a antigravity, claude-code, codex, crush, cursor, gemini-cli, github-copilot, grok, opencode` (or via specific .experimental etc) status=inspect-then-install; selector=named. Restart agent after install per docs.
