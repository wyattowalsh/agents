...
trust_tier=curated-trust-gated; status=install-now-after-trust-gate; provenance=verified-install-command; risks=wshobson/agents residual for API specs, uv workflows, changelog automation, and debugging strategies. `changelog-automation` may suggest git/release commands; review generated recipes before running them.; policy=Install only after trust gate; audit again before repo promotion or use `/review source`.; evidence=Curated wshobson/agents install/inspect batches in authoring catalog (multiple residual groups); upstream https://github.com/wshobson/agents (docs/agent-skills.md, plugins/*/skills).
