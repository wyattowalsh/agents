...
trust_tier=curated-trust-gated; status=inspect-then-install; provenance=verified-install-command; risks=Deployment pipeline design patterns. Risks around secrets, approvals, rollback.; policy=Install only after trust gate; audit again before repo promotion or use `/review source`.; evidence=Curated wshobson/agents install/inspect batches in authoring catalog (multiple residual groups); upstream https://github.com/wshobson/agents (docs/agent-skills.md, plugins/*/skills).
