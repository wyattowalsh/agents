...
SLO implementation; risks of alert fatigue, bad SLI definitions. From wshobson/agents (large multi-harness marketplace, 84 plugins/156 skills; one Markdown source to 5 harnesses).
...
Install: `npx skills add wshobson/agents --skill slo-implementation -y -g -a antigravity claude-code ...` status=inspect-then-install; selector=named. Review outputs for safety (esp. generators, k8s, tf, secrets).
...
Similar patterns in trailofbits, vercel, local skills/* or other curated (e.g. python from modern-python, k8s from hashicorp). General slo implementation instruction.
