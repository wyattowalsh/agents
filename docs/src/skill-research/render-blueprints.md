...
Install: `npx skills add render-oss/skills --skill render-blueprints -y -g -a antigravity, claude-code, codex, crush, cursor, gemini-cli, github-copilot, grok, opencode` status=inspect-then-install; selector=named; requires RENDER_API_KEY or MCP server config for full function; Git remote recommended for Blueprints.
