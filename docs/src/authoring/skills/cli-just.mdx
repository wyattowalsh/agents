...
notes: "Install only `cli-just` from `PaulRBerg/agent-skills` for Justfile and `just` command-runner workflows. The source at audited HEAD `d3f5540ed2fc0fa07f802bd925e06b9387cbe90f` exposes 26 skills, has MIT licensing, and the inspected `cli-just` skill has no observed hooks, `allowed-tools`, or skill-local scripts. Its references are documentation and examples, but include dotenv loading, package-manager, publish, network, and destructive cleanup snippets; review generated Just recipes as executable project code before running them. When both the repo-owned `/justfile` skill and curated `cli-just` are available, prefer `/justfile` for Just create/edit/migrate/check; keep `cli-just` as optional curated install."
...
When both the repo-owned `/justfile` skill and curated `cli-just` are available, prefer `/justfile` for Just create/edit/migrate/check; keep `cli-just` as optional curated install.
