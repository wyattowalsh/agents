...
{/* GENERATED-AUTHORING: source=skills/orchestrator/SKILL.md; edit SKILL.md then re-run authoring sync */}
...
When the macro graph assigns nodes to **Grok Build**, the parent still owns waves, gates, and synthesis. Delegate execution via `/grok-delegate` — native `grok -p`, `-r`, worktrees, and leader only (no custom wrapper). Pre-flight: `bash scripts/preflight.sh` before fleet dispatch. Parent dispatches parallel Grok nodes as independent bash subprocesses (Pattern A/E). Tune in-flight branches with `-r <sessionId>` after parent gates fail.
