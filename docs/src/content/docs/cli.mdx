...
Each eval file must contain three required fields: `skills` (a non-empty list of strings matching existing skill directories), `query` (a non-empty string), and `expected_behavior` (a non-empty list of strings).
