...
| `wagents doctor` | Sanity-check Python, Node/pnpm compatibility, docs deps, pre-commit, and Playwright |
...
| `wagents skills <subcommand>` | Index, search, read, and diagnose local skills on demand |
...
Diagnose your local environment and toolchain. Checks for required tools, correct Python version, docs Node/pnpm compatibility, docs dependency status, pre-commit availability, and Playwright availability.
...
| Installed Node version satisfies `docs/package.json#engines.node` | `warn` |
| Installed pnpm version matches `docs/package.json#packageManager` | `warn` |
| `docs/node_modules` present and fresh | `warn` |
| `pre-commit` on PATH when `.pre-commit-config.yaml` exists | `warn` |
...
Index and search repository, installed, and plugin skills without loading every skill description into startup context. Use this when local skill inventory exceeds an agent's context budget.
