---
title: "fetch"
description: "Core URL fetcher from the MCP reference servers. Retrieves web pages and returns content in multiple formats."
page_kind: "mcp"
...
<Badge text="MCP" variant="note" /> <Badge text="Web Fetching and Extraction" variant="caution" /> <Badge text="HTTP Fetch" variant="default" /> <Badge text="stdio" variant="note" />
...
`http://127.0.0.1:46683/mcp/fetch` via the local control plane. 
