---
title: "docs-steward"
description: "Maintains docs across Starlight, Docusaurus, Fumadocs, Sphinx themes, and MkDocs. Detects stacks, syncs artifacts, runs health checks, refreshes latest versions, and plans migrations."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="docs-steward" variant="note" /> <Badge text="2088 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Custom" variant="tip" />

> Maintains docs across Starlight, Docusaurus, Fumadocs, Sphinx themes, and MkDocs. Detects stacks, syncs artifacts, runs health checks, refreshes latest versions, and plans migrations. Use when docs frameworks or pipelines change. NOT for backend/API work or creating skills, agents, or MCP servers.
...
**Use:** `/docs-steward` `[auto|init [framework]|init-sync [framework]|sync|enhance|maintain|research|framework <name> [sync|enhance|maintain|research|migrate]|matrix|migrate <from> -> <to>]`
...
This page is generated from the repository file `skills/docs-steward/SKILL.md` by `wagents docs generate`.
</Aside>
