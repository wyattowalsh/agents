...
description: "Prompt engineering for any AI system. Craft, analyze, convert between models, evaluate with rubrics. Use for system prompts, agent prompts, tool defs. NOT for running prompts or building agents."
---
...
<Badge text="prompt-engineer" variant="note" /> <Badge text="2644 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" />

> Prompt engineering for any AI system. Craft, analyze, convert between models, evaluate with rubrics. Use for system prompts, agent prompts, tool defs. NOT for running prompts or building agents.
...
Comprehensive prompt and context engineering. Every recommendation is grounded in research. The skill distinguishes between context engineering (designing the full context window) and prompt engineering (writing the instruction text), and adapts all techniques to the target model class.

<Aside type="caution" title="Model-class awareness">
Instruction-following models and reasoning models respond differently to the same techniques. Techniques that help instruction-followers can actively hurt reasoning models (Prompting Inversion). Always detect model class first.
</Aside>
...
<Aside type="note" title="Shared preflight">
All modes now run a shared preflight: ingest the prompt, detect model class, identify deployment context and trust boundary, trigger context-management references when needed, and refuse out-of-scope “run the prompt/build the agent” requests.
</Aside>
...
1. **Requirements gathering** -- Ask targeted questions: task specifics, target model (detect model class), deployment context (single-turn, chat, agent loop, RAG pipeline), output format, failure modes to prevent, and input trust level.

2. **Architecture selection** -- Based on deployment context, select a pattern: single-turn (4-block: Context/Task/Constraints/Output), multi-turn (conversation-aware with state management), agent (ReAct with 3-instruction pattern), RAG (grounding with citation patterns), multi-agent (orchestration with role isolation).

3. **Draft prompt** -- Write using the selected architecture with XML tags as default structuring format (cross-model compatible). Apply model-class-specific guidance from `references/model-playbooks.md`.

4. **Structure for cacheability** -- Static content early (system instructions, role definitions, tool descriptions), dynamic content late (user input, retrieved documents). Enables 50-90% cost reduction via prefix caching.

5. **Harden** -- Run through `references/hardening-checklist.md`: injection resistance for untrusted input, safety constraints for user-facing output, permission minimization for tool-calling.

6. **Present** -- Format per output template. Recommend Mode D (Evaluate) to build a test suite.
...
The diagnostic scores 5 dimensions:
...
Four-lens analysis examines: ambiguity, security, robustness, and efficiency. Anti-pattern scan checks against patterns in `references/anti-patterns.md`.

## Mode C: Convert

Port a prompt between model families (Claude, GPT, Gemini, Llama) while preserving intent and quality. Loads source and target playbooks, builds a conversion plan for features that map directly, require adaptation, have no equivalent, or are newly available, then validates the result with a Mode B report-only analysis.

## Mode D: Evaluate

Build an evaluation framework (does not run evaluations -- produces the eval design). Creates golden set test cases, edge cases, adversarial cases, and regression cases. Builds a scoring rubric with dimensions, levels (1-5), concrete examples, and an LLM-as-judge prompt.

## Model-Class Detection

Mandatory first step for all modes. Determines which techniques apply.

| Model Class | Examples | Key Differences |
|-------------|----------|----------------|
| **Instruction-following** | Claude 3.5 Sonnet/Haiku, GPT-4o/4.1, Gemini 2 Flash, Llama 4 standard | Add explicit CoT scaffolding, 3-5 few-shot examples, more structure helps |
| **Reasoning** | Claude 4.x (extended thinking), GPT-5.x, Gemini 3 (thinking), o3/o4-mini | Never add external CoT (Prompting Inversion), minimal examples, concise goals over verbose steps |

<Aside type="danger" title="Prompting Inversion">
Never recommend chain-of-thought prompting for reasoning models. External CoT degrades performance for models with internal reasoning capabilities.
</Aside>

## Core Principles

- **Context engineering, not just prompting** -- Consider the full context window: system prompt, conversation history, tool results, retrieved documents. Four pillars: write, select, compress, isolate.
- **Evidence-based recommendations** -- Cite specific sources. Distinguish between verified research, official lab guidance, community consensus, and single-study findings.
- **Empirical iteration** -- Prompts are hypotheses, not solutions. Recommend eval frameworks for any non-trivial prompt.
- **Avoid over-specification** -- Beyond specificity threshold S\*~0.5, additional detail degrades performance quadratically (UCL, Jan 2026; single-study heuristic, not hard threshold).
...
[Download from GitHub](https://raw.githubusercontent.com/wyattowalsh/agents/main/skills/prompt-engineer/SKILL.md)
...
---
[View source on GitHub](https://github.com/wyattowalsh/agents/blob/main/skills/prompt-engineer/SKILL.md)
