...
<Badge text="prospector" variant="note" /> <Badge text="2341 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Custom" variant="tip" />
...
This page is generated from the repository file `skills/prospector/SKILL.md` by `wagents docs generate`.
</Aside>
...
<Tabs>
  <TabItem label="Seeded">

```bash
/prospector developer tools for AI agents
```

Runs a focused discovery pass for a specific niche and returns ranked opportunity cards.
...
Runs free-roam discovery against community sources to find themes before clustering.
...
Runs the expert panel workflow on a discovered opportunity before moving it forward.

  </TabItem>
</Tabs>

<Aside type="tip" title="Recommended first workflow">
Start with a seeded run, then review candidates with `/prospector list --tier strong`, and only then use `evaluate` for the top opportunities.
</Aside>
...
2. **Review strongest candidates**

   ```bash
   /prospector list --tier strong
   ```

3. **Deep-assess one idea**
...
<CardGrid>
  <LinkCard title="honest-review" href="/skills/honest-review/" description="Use for evaluating existing live products instead of discovery." />
  <LinkCard title="wargame" href="/skills/wargame/" description="Use for competitive and strategic analysis after identifying a promising idea." />
  <LinkCard title="research" href="/skills/research/" description="Use for broad, general-purpose research outside Prospector's discovery pipeline." />
</CardGrid>
