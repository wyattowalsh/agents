...
description: "Guide for writing GraphQL operations (queries, mutations, fragments) following best practices."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="graphql-operations" variant="note" /> <Badge text="521 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.0" variant="success" /> <Badge text="apollographql" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add apollographql/skills --skill graphql-operations -y -g
```
