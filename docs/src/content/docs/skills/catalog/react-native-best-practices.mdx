...
description: "Provides React Native performance optimization guidelines for FPS, TTI, bundle size, memory leaks, re-renders, and animations."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="react-native-best-practices" variant="note" /> <Badge text="1145 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="Callstack" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add callstackincubator/agent-skills --skill react-native-best-practices -y -g
```
...
| Display Source | `callstackincubator/agent-skills` |
| Source Kind | `github` |
