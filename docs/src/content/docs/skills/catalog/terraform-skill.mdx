...
description: "Use when writing, reviewing, or debugging Terraform/OpenTofu modules, tests, CI, scans, or state ops - diagnoses failure mode (identity churn, secrets, blast radius, CI drift, state corruption) with…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="terraform-skill" variant="note" /> <Badge text="2508 words" variant="default" /> <Badge text="Apache-2.0" variant="success" /> <Badge text="v1.17.1" variant="success" /> <Badge text="Anton Babenko" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add antonbabenko/terraform-skill --skill terraform-skill -y -g
```
