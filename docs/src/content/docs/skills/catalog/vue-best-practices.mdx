...
description: "MUST be used for Vue.js tasks. Strongly recommends Composition API with `&lt;script setup&gt;` and TypeScript as the standard approach. Covers Vue 3, SSR, Volar, vue-tsc. Load for any Vue, ."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="vue-best-practices" variant="note" /> <Badge text="1024 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v18.0.0" variant="success" /> <Badge text="github.com/vuejs-ai" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add hyf0/vue-skills --skill vue-best-practices -y -g
```
