...
description: "Build AI agents with Pydantic AI — tools, capabilities, structured output, streaming, testing, and multi-agent patterns."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="building-pydantic-ai-agents" variant="note" /> <Badge text="1289 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.0" variant="success" /> <Badge text="pydantic" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add pydantic/skills --skill building-pydantic-ai-agents -y -g
```
