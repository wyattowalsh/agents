...
description: "Idiomatic Golang design patterns — functional options, constructors, error flow and cascading, resource management and lifecycle, graceful shutdown, resilience, architecture, dependency injection,…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-design-patterns" variant="note" /> <Badge text="1365 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.3" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-design-patterns -y -g
```

**Use:** `/golang-design-patterns`
