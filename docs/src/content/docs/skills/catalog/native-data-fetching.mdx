...
description: "Use when implementing or debugging ANY network request, API call, or data fetching."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="native-data-fetching" variant="note" /> <Badge text="1436 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add expo/skills --skill native-data-fetching -y -g
```

**Use:** `/native-data-fetching`
