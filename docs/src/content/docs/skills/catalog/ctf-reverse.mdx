...
<Badge text="ctf-reverse" variant="note" /> <Badge text="0 words" variant="default" /> <Badge text="Curated External" variant="caution" /> <Badge text="inspect-then-install" variant="default" />
...
This page is generated from curated config (`config/external-skills.md`) and local inventory when installed.
 SKILL.md is not available locally; install the skill or run `uv run wagents docs research --skill ctf-reverse` to refresh research context.
</Aside>
