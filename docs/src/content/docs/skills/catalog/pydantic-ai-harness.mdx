...
description: "Extend Pydantic AI agents with batteries-included capabilities from pydantic-ai-harness -- currently Code Mode, which collapses many tool calls into one sandboxed Python execution."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="pydantic-ai-harness" variant="note" /> <Badge text="610 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v0.1.0" variant="success" /> <Badge text="pydantic" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add pydantic/skills --skill pydantic-ai-harness -y -g
```
