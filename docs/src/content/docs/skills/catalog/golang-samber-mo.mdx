---
title: "golang-samber-mo"
description: "Monadic types for Golang using samber/mo — Option, Result, Either, Future, IO, Task, and State types for type-safe nullable values, error handling, and functional composition with pipeline…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-samber-mo" variant="note" /> <Badge text="1295 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.3" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-samber-mo -y -g
```

**Use:** `/golang-samber-mo`
