...
description: "Guide for using Apollo MCP Server to connect AI agents with GraphQL APIs."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="apollo-mcp-server" variant="note" /> <Badge text="748 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.1" variant="success" /> <Badge text="apollographql" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add apollographql/skills --skill apollo-mcp-server -y -g
```
