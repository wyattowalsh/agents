...
description: "Solves CTF challenges by performing first-pass triage, identifying the dominant category, and routing execution to the right specialized ctf-* skill."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="solve-challenge" variant="note" /> <Badge text="1167 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add ljagiello/ctf-skills --skill solve-challenge -y -g
```
