...
<Badge text="chrome-devtools-cli" variant="note" /> <Badge text="527 words" variant="default" /> <Badge text="Apache-2.0" variant="success" /> <Badge text="v0.23.0" variant="success" /> <Badge text="Google LLC" variant="caution" /> <Badge text="Repo-owned" variant="tip" />
...
**Install:**
```bash
npx skills add github:wyattowalsh/agents --skill chrome-devtools-cli -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode
```
...
Composed compose-custom-wave-1 from `skills/chrome-devtools-cli/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- Verify the CLI exists with `chrome-devtools --help` or provide setup guidance if it does not.

2. **Step** -- Navigate or select a page.

3. **Step** -- Take a snapshot to obtain `uid`s.

4. **Step** -- Act with focused commands such as `click`, `fill`, `press_key`, or `upload_file`.

5. **Step** -- Verify with a fresh snapshot, console messages, network requests, or screenshot.
