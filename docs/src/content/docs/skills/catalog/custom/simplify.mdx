...
<Badge text="simplify" variant="note" /> <Badge text="1634 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Repo-owned" variant="tip" />
...
**Install:**
```bash
npx skills add github:wyattowalsh/agents --skill simplify -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode
```

**Use:** `/simplify` `[mode] [target]`
...
Composed compose-custom-wave-7 from `skills/simplify/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
[Download from GitHub](https://raw.githubusercontent.com/wyattowalsh/agents/main/skills/simplify/SKILL.md)
...
---
[View source on GitHub](https://github.com/wyattowalsh/agents/blob/main/skills/simplify/SKILL.md)
