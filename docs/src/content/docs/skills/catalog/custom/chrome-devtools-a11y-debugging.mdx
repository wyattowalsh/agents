...
<Badge text="chrome-devtools-a11y-debugging" variant="note" /> <Badge text="535 words" variant="default" /> <Badge text="Apache-2.0" variant="success" /> <Badge text="v0.23.0" variant="success" /> <Badge text="Google LLC" variant="caution" /> <Badge text="Repo-owned" variant="tip" />
...
**Install:**
```bash
npx skills add github:wyattowalsh/agents --skill chrome-devtools-a11y-debugging -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode
```
...
Composed compose-custom-wave-1 from `skills/chrome-devtools-a11y-debugging/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- Navigate to the target page and wait for the relevant UI state.

2. **Step** -- Run `lighthouse_audit` in `navigation` mode for a baseline when page reload is safe.

3. **Step** -- Inspect browser issues with `list_console_messages` using `types: ["issue"]` and preserved messages.

4. **Step** -- Capture `take_snapshot` and check headings, landmarks, names, and order.

5. **Step** -- Test keyboard navigation with `press_key` and fresh snapshots.

6. **Step** -- Use snippets only for data missing from the accessibility tree, such as tap target dimensions or manual contrast estimates.

7. **Step** -- Report failures with selector or `uid`, observed behavior, expected behavior, and remediation.
