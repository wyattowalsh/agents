...
<SkillPageHeader
  name="skill-router"
  description="Route tasks to local skills. Use when choosing skills, recovering omitted skills after context warnings, or preparing a small skill context packet. NOT for install, authoring, or audit workflows."
  installCommand={`npx skills add github:wyattowalsh/agents --skill skill-router -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/skill-router &lt;search|context|read|doctor&gt; [query]"
  badges={[{ text: "skill-router", variant: "note" }, { text: "754 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v1.0.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-6 from `skills/skill-router/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
````yaml title="skills/skill-router/SKILL.md"
---
name: skill-router
description: >-
