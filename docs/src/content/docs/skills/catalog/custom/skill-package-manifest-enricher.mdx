...
<SkillPageHeader
  ...
  description="Enrich portable skill ZIP manifests with compatibility and eval metadata. Use before packaging or catalog promotion. NOT for live installs."
  installCommand={`npx skills add github:wyattowalsh/agents --skill skill-package-manifest-enricher -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
