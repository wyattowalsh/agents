...
page_kind: "skill"
source_kind: "custom"
asset_id: "opencode-ensemble"
composed: true
docs_density: standard
license: "MIT"
trust_tier: "repo"
metadata.author: "hueyexe"
metadata.version: "1.0.0"
research_slug: "opencode-ensemble"
...
<SkillPageHeader
  ...
  description="Use when coordinating OpenCode Ensemble teams, delegating independent coding work, reviewing teammate output, or running staged parallel waves."
  installCommand={`npx skills add github:wyattowalsh/agents --skill opencode-ensemble -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/opencode-ensemble"
  badges={[{ text: "opencode-ensemble", variant: "note" }, { text: "1464 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v1.0.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-5 from `skills/opencode-ensemble/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- Decide whether parallelism is justified.

2. **Step** -- Create a team with `team_create`.

3. **Step** -- Add tasks with `team_tasks_add`; use `depends_on` for sequencing.

4. **Step** -- Spawn teammates one at a time with `team_spawn`; after spawning, they run asynchronously in parallel.

5. **Step** -- Use `worktree: false` for read-only `explore` teammates.

6. **Step** -- Use `plan_approval: true` for risky implementation work.

7. **Step** -- Hold a lead synthesis gate before starting a dependent wave.

8. **Step** -- Wait for teammate messages instead of polling status repeatedly.

9. **Step** -- Read full results with `team_results` when messages are truncated or consequential.

10. **Step** -- Shut down completed teammates with `team_shutdown`.

11. **Step** -- Merge branches with `team_merge`; inspect the diff before trusting it.

12. **Step** -- Run project verification before `team_cleanup` and before claiming done.
...
````yaml title="skills/opencode-ensemble/SKILL.md"
---
name: opencode-ensemble
