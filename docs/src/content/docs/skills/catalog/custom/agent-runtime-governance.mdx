...
page_kind: "skill"
source_kind: "custom"
asset_id: "agent-runtime-governance"
composed: true
...
research_slug: "agent-runtime-governance"
...
<SkillPageHeader
  ...
  description="Audit runtime controls for tool permissions, approvals, memory, telemetry, evals, rollout, and containment. Use when reviewing tool-bearing agent systems."
  installCommand={`npx skills add github:wyattowalsh/agents --skill agent-runtime-governance -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/agent-runtime-governance &lt;mode&gt; [system-or-path]"
  badges={[{ text: "agent-runtime-governance", variant: "note" }, { text: "941 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-1 from `skills/agent-runtime-governance/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- Define the agent’s job, users, data sensitivity, and external effects.

2. **Step** -- Inventory tools by capability: read-only, write, destructive, financial,

3. **Step** -- Map approval gates to consequence, reversibility, and confidence.

4. **Step** -- Define memory scope, retention, redaction, and invalidation rules.

5. **Step** -- Require telemetry for tool calls, decisions, approval outcomes, and failures.

6. **Step** -- Build evals around unsafe tool use, stale memory, missing approval, and

7. **Step** -- Define rollout gates, rollback criteria, and operator evidence for changes

8. **Step** -- Return a governance matrix with owners and enforcement points.
