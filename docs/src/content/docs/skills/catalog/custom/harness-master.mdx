...
description: "Audit harness configs, discover gaps, usage signals, apply approved fixes. Use when tuning Claude, Codex, Copilot, Cursor, Gemini, Grok, OpenCode, or Cherry. NOT agents, MCP, or app telemetry."
page_kind: "skill"
source_kind: "custom"
asset_id: "harness-master"
composed: true
...
<SkillPageHeader
  name="harness-master"
  description="Audit harness configs, discover gaps, usage signals, apply approved fixes. Use when tuning Claude, Codex, Copilot, Cursor, Gemini, Grok, OpenCode, or Cherry. NOT agents, MCP, or app telemetry."
  installCommand={`npx skills add github:wyattowalsh/agents --skill harness-master -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/harness-master [discover|audit|usage|apply|install|resume|list] [harness|all] [project|global|both]"
  badges={[{ text: "harness-master", variant: "note" }, { text: "3155 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v1.1.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-3 from `skills/harness-master/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- Parse `$ARGUMENTS` into harnesses, level, `all`, install intent, apply intent, and unresolved tokens.

2. **Step** -- If unresolved tokens remain, ask a single clarification question before continuing.

3. **Step** -- If the request is `discover ...`, gap/missing/expand intent, or find-best-for-harness NL, run Discover (infer depth via `classify_intent.py` or discovery-pipeline.md).

4. **Step** -- If the request is `usage ...`, run Usage Review only.

5. **Step** -- If the request is `install ...`, run Install Guidance only.

6. **Step** -- If ad-hoc "find skill for X", redirect to find-skills. If create skill, redirect to skill-creator. If open-ended non-harness research, redirect to `/research`.

7. **Step** -- If the request is `apply ...` or an approval phrase, run Apply Approved only if the matching dry-run review already exists in this session. Otherwise rerun audit first.

8. **Step** -- If the request includes unsupported harnesses, refuse cleanly and list the supported set.

9. **Step** -- If harnesses are missing (audit/usage), ask only for harnesses or `all`.

10. **Step** -- If the level is missing (audit/usage), ask only for `project`, `global`, or `both`.

11. **Step** -- Otherwise run Audit in dry-run mode.
...
1. **Step** -- Route explicit mode tokens first: `discover`, `usage`, `install`, `apply`, `resume`, `list`.

2. **Step** -- Route gap/missing/expand/find-best NL to Discover depths (not Audit).

3. **Step** -- Route complete harness/level scopes to Audit when config tune/fix intent and no discover signal.

4. **Step** -- Route usage/cost/token NL to Usage Review.

5. **Step** -- Route approvals to Apply Approved only when the immediately preceding dry-run audit matches the requested scope.
...
| Field | Value |
| ----- | ----- |
| Compatibility | Dry-run audit first. Web access recommended. Python 3.12+ for discovery scripts. |
  </TabItem>
</Tabs>
...
````yaml title="skills/harness-master/SKILL.md"
---
name: harness-master
description: >-
