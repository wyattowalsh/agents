...
research_slug: "database-architect"
...
<Badge text="database-architect" variant="note" /> <Badge text="1303 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Repo-owned" variant="tip" />
...
**Install:**
```bash
npx skills add github:wyattowalsh/agents --skill database-architect -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode
```

**Use:** `/database-architect` `&lt;mode&gt; [target]`
...
Composed compose-custom-wave-2 from `skills/database-architect/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
[Download from GitHub](https://raw.githubusercontent.com/wyattowalsh/agents/main/skills/database-architect/SKILL.md)
...
---
[View source on GitHub](https://github.com/wyattowalsh/agents/blob/main/skills/database-architect/SKILL.md)
