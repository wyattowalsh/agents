...
<Badge text="honest-review" variant="note" /> <Badge text="MIT" variant="success" /> <Badge text="v5.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" />
...
Also includes: 10 creative lenses, review history, HTML dashboard, degraded mode, state management, CI integration, and hooks.
...
| "fix" / "apply" approved findings | Post-review fix pass |
...
1. **Triage (Wave 0)** -- Risk-stratify files as HIGH/MEDIUM/LOW. Run `uv run python skills/honest-review/scripts/project-scanner.py` for project profiling. Determine specialist triggers (security, observability, requirements).
...
The current source contract is reasoning-first and read-only by default: review output stops at findings, strengths, and recommendations, and implementation summaries only appear after explicit approval and a separate fix pass.
</Aside>
...
Review history is persisted to the active agent's honest-reviews state directory via `skills/honest-review/scripts/review-store.py`:
...
| Script | Purpose |
|--------|---------|
| `skills/honest-review/scripts/project-scanner.py` | Wave 0 triage -- project profiling with dependency graph construction and fan-in risk scoring |
| `skills/honest-review/scripts/finding-formatter.py` | Wave 3 Judge -- normalize findings to JSON, supports `--format sarif` and `--format conventional` |
| `skills/honest-review/scripts/review-store.py` | State management -- save, load, list, diff review history (schema v2 with reasoning tracking) |
| `skills/honest-review/scripts/learnings-store.py` | Learning loop -- add, check, list, clear false-positive dismissals per project |
...
<CardGrid>
  <LinkCard title="skill-creator" href="/skills/catalog/custom/skill-creator/" description="Create, improve, and audit AI agent skills with 13 proven structural patterns." />
  <LinkCard title="orchestrator" href="/skills/catalog/custom/orchestrator/" description="Build and deploy parallel execution via subagent waves, agent teams, and multi-wave pipelines." />
  <LinkCard title="add-badges" href="/skills/catalog/custom/add-badges/" description="Scan a codebase to detect languages, frameworks, CI/CD pipelines, package managers, and tools, then generate and insert shields." />
  <LinkCard title="wargame" href="/skills/catalog/custom/wargame/" description="Domain-agnostic strategic decision analysis and wargaming with AI-controlled actors." />
</CardGrid>
