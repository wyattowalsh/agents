...
page_kind: "skill"
source_kind: "custom"
asset_id: "schema-evolution-planner"
composed: true
...
research_slug: "schema-evolution-planner"
...
<SkillPageHeader
  ...
  description="Plan zero-downtime schema changes across code, data backfills, and cutovers. Use for expand-contract database changes. NOT for fresh schema design or DBA ops."
  installCommand={`npx skills add github:wyattowalsh/agents --skill schema-evolution-planner -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/schema-evolution-planner &lt;mode&gt; [change]"
  badges={[{ text: "schema-evolution-planner", variant: "note" }, { text: "1158 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-6 from `skills/schema-evolution-planner/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
