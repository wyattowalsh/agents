...
<SkillPageHeader
  name="mcp-creator"
  description="Build MCP servers with FastMCP v3. Research, scaffold, implement, test, deploy. Use when creating MCP servers or integrating APIs via MCP. NOT for REST APIs, CLI tools, or non-MCP integrations."
  installCommand={`npx skills add github:wyattowalsh/agents --skill mcp-creator -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/mcp-creator &lt;service or API to integrate&gt;"
  badges={[{ text: "mcp-creator", variant: "note" }, { text: "2531 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v2.0.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-1 from `skills/mcp-creator/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
````yaml title="skills/mcp-creator/SKILL.md"
---
name: mcp-creator
description: >-
