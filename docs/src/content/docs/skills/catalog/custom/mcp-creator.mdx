...
<Badge text="mcp-creator" variant="note" /> <Badge text="2489 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v2.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" />
...
**Install:**
```bash
npx skills add github:wyattowalsh/agents --skill mcp-creator -y -g --agent claude-code --agent codex --agent gemini-cli --agent antigravity --agent github-copilot --agent opencode
```
...
<Aside type="tip" title="Live docs first">
Before implementation, fetch current FastMCP v3 docs via Context7 or WebFetch (`https://gofastmcp.com/llms-full.txt`). If live docs contradict bundled references, live docs win.
</Aside>
...
<Aside type="note" title="Validation">
`wagents validate` checks skills and agents only, NOT MCP servers. Use `uv run python -c "from server import mcp; print(mcp.name)"` and `fastmcp list` for MCP server validation.
</Aside>
...
<CardGrid>
  <LinkCard title="skill-creator" href="/skills/catalog/custom/skill-creator/" description="Create, improve, and audit AI agent skills. Applies 13 proven structural patterns, scores quality with deterministic audit, manages full lifecycle." />
</CardGrid>
