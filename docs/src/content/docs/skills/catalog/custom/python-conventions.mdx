...
<SkillPageHeader
  ...
  description="Enforce Python tooling conventions for uv, ty, Ruff, pytest, and pyproject.toml. Use when working on .py files or Python project config."
  installCommand={`npx skills add github:wyattowalsh/agents --skill python-conventions -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
...
Composed compose-custom-wave-1 from `skills/python-conventions/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- Profile before optimizing.

2. **Step** -- Use `references/performance-tips.md` for quick Python profiling and optimization patterns.

3. **Step** -- Route broad profiling, regression analysis, or performance investigation to `performance-profiler`.
...
1. **Step** -- Use pytest for Python tests and configure project test defaults in `pyproject.toml`.

2. **Step** -- Use `references/testing-patterns.md` for fixtures, markers, `tmp_path`, `monkeypatch`, parametrization, and coverage policy.

3. **Step** -- Route test strategy, suite design, fixture architecture, or cross-language test plans to `test-architect`.
...
````yaml title="skills/python-conventions/SKILL.md"
---
name: python-conventions
description: >-
