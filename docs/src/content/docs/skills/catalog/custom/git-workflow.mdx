...
<SkillPageHeader
  name="git-workflow"
  description="Git operations: conventional commits, PR descriptions, branch strategy, conflict resolution, code archaeology, bisect. Use for git workflow tasks. NOT for code review, CI/CD, or changelogs."
  installCommand={`npx skills add github:wyattowalsh/agents --skill git-workflow -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/git-workflow &lt;mode&gt; [args]"
  badges={[{ text: "git-workflow", variant: "note" }, { text: "1356 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v1.0.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-3 from `skills/git-workflow/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
````yaml title="skills/git-workflow/SKILL.md"
---
name: git-workflow
description: >-
