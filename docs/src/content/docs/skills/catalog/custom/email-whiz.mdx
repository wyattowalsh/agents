...
<SkillPageHeader
  name="email-whiz"
  description="Gmail copilot via MCP. Triage, inbox-zero, filters, analytics, labels, cleanup. Use when managing email or automating Gmail. NOT for composing emails, calendar, or non-Gmail."
  installCommand={`npx skills add github:wyattowalsh/agents --skill email-whiz -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/email-whiz &lt;mode&gt; [options]"
  badges={[{ text: "email-whiz", variant: "note" }, { text: "3289 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-3 from `skills/email-whiz/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- **Independent tool calls go in ONE message.** Sequential independent calls is a bug.

2. **Step** -- **Phase 0 fuses with mode's first queries** — never 2 sequential rounds when fusion is possible.

3. **Step** -- **Batch ops on disjoint ID sets: parallel.** NOISE + REFERENCE + DEFER = 1 message, 3 calls.

4. **Step** -- **Analytics/Audit: mega-wave** — 10-15 search queries in 1 message.

5. **Step** -- **gmail_read_email: batch 5-8 per message** (full messages are large).

6. **Step** -- **Filter creation: parallel** after conflict check passes.

7. **Step** -- **Combo mode boundary: sequential** (data dependency). But skip Phase 0 for mode 2.

8. **Step** -- **Max 15-20 calls per message.** Beyond that, a single 429 impacts the entire bundle.

9. **Step** -- **On 429:** preserve completed results, drop to sequential with 2s gaps. 3x429 in 60s → pause 60s.
...
<!-- deduped Orchestration Patterns -->
