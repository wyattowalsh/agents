...
Composed compose-custom-wave-1 from `skills/add-badges/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
Show grouped preview with category headers. Render badges as actual ``![alt`](link)` markdown so the user can see them.
...
````yaml title="skills/add-badges/SKILL.md"
---
name: add-badges
description: >-
