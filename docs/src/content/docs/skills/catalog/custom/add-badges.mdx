...
**Install:**
```bash
npx skills add github:wyattowalsh/agents --skill add-badges -y -g --agent claude-code --agent codex --agent gemini-cli --agent antigravity --agent github-copilot --agent opencode
```
...
The public contract is dispatch-first: empty/help, full workflow, `--dry-run`, and removal-only requests route first, while Detect/Select/Present/Insert remain the internal execution phases.
</Aside>
...
<CardGrid>
  <LinkCard title="honest-review" href="/skills/catalog/custom/honest-review/" description="Confidence-scored code review with evidence validation for session diffs and full codebase audits." />
</CardGrid>
