...
<SkillPageHeader
  name="host-panel"
  description="Facilitate research-grounded panels in roundtable, Oxford, and Socratic formats. Use when exploring contested topics from multiple angles. NOT for Q&amp;A, code review, or real human opinion simulation."
  installCommand={`npx skills add github:wyattowalsh/agents --skill host-panel -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/host-panel &quot;topic&quot; [format] [num-experts]"
  badges={[{ text: "host-panel", variant: "note" }, { text: "1381 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v1.0.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-1 from `skills/host-panel/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
1. **Step** -- **Parse.** Preserve the public invocation contract. Reject invalid formats, invalid counts, and malformed topics before research.

2. **Step** -- **Diagnose.** Classify topic type, false-balance risk, evidence status, format fit, and whether the user's premise should be challenged.

3. **Step** -- **Ground.** Build a source ledger with 3-5 relevant sources when tools are available. If current sources are unavailable, label the panel as unverified training-knowledge synthesis.

4. **Step** -- **Map perspectives.** Discover traditions from the topic and sources before personas. Prefer methodology diversity over theatrical variety.

5. **Step** -- **Run independent first positions.** Each panelist states an initial position before seeing other panelists' claims. Subsequent rounds critique claims, not people.

6. **Step** -- **Moderate against collapse.** Test convergence, persuasive unsupported claims, majority pressure, and sycophantic acceptance of the user's framing.

7. **Step** -- **Synthesize by trajectory.** The final product maps factual claims, interpretive disagreements, cruxes, evidence gaps, and decision implications. It is not a vote.
...
````yaml title="skills/host-panel/SKILL.md"
---
name: host-panel
description: >-
