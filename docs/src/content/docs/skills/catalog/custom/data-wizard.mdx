...
<SkillPageHeader
  name="data-wizard"
  description="Analyze data and guide ML: EDA, model selection, feature engineering, stats, visualization, MLOps. Use for data work. NOT for ETL, database design (database-architect), or frontend viz code."
  installCommand={`npx skills add github:wyattowalsh/agents --skill data-wizard -y -g --agent antigravity --agent claude-code --agent codex --agent crush --agent cursor --agent gemini-cli --agent github-copilot --agent grok --agent opencode`}
  usage="/data-wizard &lt;mode&gt; &lt;data|task|question&gt; [options]"
  badges={[{ text: "data-wizard", variant: "note" }, { text: "1643 words", variant: "default" }, { text: "MIT", variant: "success" }, { text: "v1.0.0", variant: "tip" }, { text: "Repo-owned", variant: "tip" }]}
/>
...
Composed compose-custom-wave-1 from `skills/data-wizard/SKILL.md` with full SKILL disclosure. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel).
</Aside>
...
<Aside type="tip" title="Viz pipeline">
Executable charts flow **probe/profile → viz-planner → viz-renderer → dashboard-builder** (optional HTML EDA dashboard). Use `viz plan`, `viz render`, and `viz dashboard` sub-modes after profiling data.
</Aside>
...
````yaml title="skills/data-wizard/SKILL.md"
---
name: data-wizard
description: >-
