...
description: "Find and fix issues from Sentry using MCP. Use when asked to fix Sentry errors, debug production issues, investigate exceptions, or resolve bugs reported in Sentry."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="sentry-fix-issues" variant="note" /> <Badge text="1062 words" variant="default" /> <Badge text="Apache-2.0" variant="success" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add getsentry/sentry-for-ai --skill sentry-fix-issues -y -g
```
