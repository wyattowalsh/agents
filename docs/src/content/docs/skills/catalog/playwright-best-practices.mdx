...
description: "Use when writing Playwright tests, fixing flaky tests, debugging failures, implementing Page Object Model, configuring CI/CD, optimizing performance, mocking APIs, handling authentication or OAuth,…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="playwright-best-practices" variant="note" /> <Badge text="1630 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1" variant="success" /> <Badge text="currents.dev" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add currents-dev/playwright-best-practices-skill --skill playwright-best-practices -y -g
```
