...
<SkillPageHeader
  ...
  usage="/ctf-forensics"
  badges={[{ text: "ctf-forensics", variant: "note" }, { text: "47 words", variant: "default" }, { text: "Inspect first", variant: "caution" }, { text: "inspect-then-install", variant: "default" }]}
/>
...
Composed compose-external-wave-3 using authoring SSOT (`docs/src/authoring/skills/ctf-forensics.mdx`) + research. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel). Run `/review source` before using in production or promoting.
</Aside>

## What It Does

From ljagiello/ctf-skills repo: agent skills for CTF categories. ctf-forensics covers techniques from writeups (e.g. ai-ml: adversarial, jailbreaks, model extraction; pwn/reverse: binary; web: exploitation; etc). Includes scripts/ for env. MIT license stated, SECURITY.md with responsible use. Use only for authorized CTF/lab/security-research.
...
Entry maintained in authoring SSOT `docs/src/authoring/skills/ctf-forensics.mdx` and `docs/public/generated-registries/skills-catalog-index.json`; research context is advisory.

## Install Prerequisites

npx skills add ljagiello/ctf-skills --skill ctf-forensics ... ; or full group; pre-run installer dry-run; status=inspect-then-install.
