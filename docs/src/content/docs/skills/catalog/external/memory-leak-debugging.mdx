...
Composed compose-external-wave-1 using authoring SSOT (`docs/src/authoring/skills/memory-leak-debugging.mdx`) + research. Do not run `wagents docs generate` on this file (HAND-MAINTAINED sentinel). Run `/review source` before using in production or promoting.
</Aside>
