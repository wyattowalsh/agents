---
title: "inngest-setup"
description: "Use when adding durable execution to a TypeScript project — building retry-safe webhook handlers, background jobs that survive crashes, scheduled tasks, or long-running workflows that outlive a…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="inngest-setup" variant="note" /> <Badge text="1514 words" variant="default" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add inngest/inngest-skills --skill inngest-setup -y -g
```
