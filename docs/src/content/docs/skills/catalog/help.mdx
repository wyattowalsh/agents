---
title: "help"
description: "Grok documentation and configuration help. Use when users ask about setup, configuration, MCP servers, authentication, skills, slash commands, keyboard shortcuts, or any Grok feature."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="help" variant="note" /> <Badge text="252 words" variant="default" /> <Badge text="Installed" variant="caution" />
...
1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `/Users/ww/.grok/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `/Users/ww/.grok/docs/user-guide/`. The available guides are:
   ...
   `/Users/ww/.grok/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `/Users/ww/.grok/config.toml` with search_replace.

4. To **create a skill** for the user, create `/Users/ww/.grok/skills/<name>/SKILL.md`
   (read `/Users/ww/.grok/docs/user-guide/08-skills.md` for the SKILL.md format).
````
