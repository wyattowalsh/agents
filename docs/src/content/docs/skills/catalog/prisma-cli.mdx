---
title: "prisma-cli"
description: "Prisma ORM CLI commands reference covering init, generate, migrate, db, dev, studio, validate, format, debug, and mcp. Use for ORM/database CLI workflows, not Prisma Compute app deployment."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="prisma-cli" variant="note" /> <Badge text="835 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v7.6.0" variant="success" /> <Badge text="prisma" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add prisma/skills --skill prisma-cli -y -g
```
