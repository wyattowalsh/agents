...
description: "Golang CLI command tree library using spf13/cobra — cobra."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-spf13-cobra" variant="note" /> <Badge text="1144 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.0" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-spf13-cobra -y -g
```
