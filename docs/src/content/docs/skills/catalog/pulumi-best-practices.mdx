...
description: "Load when the user is writing, reviewing, or debugging Pulumi TypeScript/Python programs; asks about Output&lt;T&gt; or apply() usage; wants to create ComponentResource classes; needs to refactor resources…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="pulumi-best-practices" variant="note" /> <Badge text="1912 words" variant="default" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add pulumi/agent-skills --skill pulumi-best-practices -y -g
```
