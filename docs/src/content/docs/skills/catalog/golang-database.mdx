...
description: "Comprehensive guide for Go database access."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-database" variant="note" /> <Badge text="1442 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.2" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-database -y -g
```
