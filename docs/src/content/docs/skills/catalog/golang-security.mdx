...
description: "Security best practices and vulnerability prevention for Golang."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-security" variant="note" /> <Badge text="1642 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.7" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-security -y -g
```
