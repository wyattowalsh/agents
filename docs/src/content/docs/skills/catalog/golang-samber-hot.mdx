...
description: "In-memory caching in Golang using samber/hot — eviction algorithms (LRU, LFU, TinyLFU, W-TinyLFU, S3FIFO, ARC, TwoQueue, SIEVE, FIFO), TTL, cache loaders, sharding, stale-while-revalidate, missing…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-samber-hot" variant="note" /> <Badge text="864 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.3" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-samber-hot -y -g
```

**Use:** `/golang-samber-hot`
