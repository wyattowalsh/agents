...
description: "INVOKE THIS SKILL when your Deep Agent needs memory, persistence, or filesystem access."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="deep-agents-memory" variant="note" /> <Badge text="1050 words" variant="default" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add langchain-ai/langchain-skills --skill deep-agents-memory -y -g
```
...
``` </python> <typescript> Default StateBackend stores files ephemerally within a thread. ```typescript import { createDeepAgent } from "deepagents";
