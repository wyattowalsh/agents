---
title: "here-now"
description: "here.now lets agents publish websites and store private files in cloud Drives. Use Sites to publish HTML, documents, images, PDFs, videos, and static files to live URLs at {slug}.here."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="here-now" variant="note" /> <Badge text="1200 words" variant="default" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add heredotnow/skill --skill here-now -y -g
```
...
| Display Source | `heredotnow/skill` |
| Source Kind | `github` |
