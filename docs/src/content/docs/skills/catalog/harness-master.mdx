...
<Badge text="harness-master" variant="note" /> <Badge text="3426 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Custom" variant="tip" />
...
- **Plannotator:** CLI + skills + hooks (no Grok npm plugin). Install: `wagents grok plannotator install`. Policy: `config/grok-plannotator-hooks.json` -> `~/.grok/hooks/plannotator.json`; shim `scripts/grok/plannotator-exit-plan-hook.sh` maps `block` -> `deny`. Curated in `config/external-skills.md`. Contrast OpenCode `@plannotator/opencode@latest`.
- **Env (not in TOML):** `GROK_WEB_FETCH`, `GROK_MEMORY`, `GROK_SUBAGENTS`, `GROK_LSP_TOOLS` — document in `config/grok-env.sh`; check with `wagents grok doctor`
