...
description: "Continuously modernize Golang code to use the latest language features, standard library improvements, and idiomatic patterns."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="golang-modernize" variant="note" /> <Badge text="1144 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v1.1.4" variant="success" /> <Badge text="samber" variant="caution" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add samber/cc-skills-golang --skill golang-modernize -y -g
```
