...
description: "INVOKE THIS SKILL for LLM-as-judge evaluation workflows on Arize: creating/updating evaluators, running evaluations on spans or experiments, tasks, trigger-run, column mapping, and continuous…"
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="arize-evaluator" variant="note" /> <Badge text="3567 words" variant="default" /> <Badge text="Installed" variant="caution" />
...
**Install:**
```bash
npx skills add github/awesome-copilot --skill arize-evaluator -y -g
```
