...
<Badge text="email-whiz" variant="note" /> <Badge text="2726 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v5.0.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Custom" variant="tip" />
...
12. NEVER delete emails. Archive by default in all modes. `gmail_delete_email` and `gmail_batch_delete_emails` are not in allowed-tools
...
16. Check session cache before Phase 0. Invalidate cache after any write operation (`gmail_create_label`, `gmail_get_or_create_label`, `gmail_create_filter`, `gmail_create_filter_from_template`, `gmail_batch_modify_emails`).
...
| Field | Value |
| ----- | ----- |
| Allowed Tools | `gmail_search_emails gmail_read_email gmail_list_email_labels gmail_create_label gmail_modify_email gmail_list_filters gmail_get_filter gmail_create_filter gmail_create_filter_from_template gmail_delete_filter gmail_batch_modify_emails gmail_get_or_create_label gmail_update_label gmail_delete_label gmail_send_email Read Grep Write` |
  </TabItem>
</Tabs>
...
Single: `gmail_modify_email`, `gmail_create_label`
Batch: `gmail_batch_modify_emails`
...
**Email deletion is disabled.** `gmail_delete_email` and `gmail_batch_delete_emails` are not in allowed-tools. Always archive instead.
