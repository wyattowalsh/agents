---
title: "mcp-creator"
description: "Build production-ready MCP servers using FastMCP v3. Guides research, scaffolding, tool/resource/prompt implementation, testing, and deployment. Targets FastMCP 3.0."
---
...
<Badge text="mcp-creator" variant="note" /> <Badge text="2489 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v2.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" />
...
| OpenAPI spec URL or file path | **Convert OpenAPI** | Phase 2 (scaffold) then `references/server-composition.md` section 6 |
| FastAPI app to convert | **Convert FastAPI** | Phase 2 (scaffold) then `references/server-composition.md` section 7 |
