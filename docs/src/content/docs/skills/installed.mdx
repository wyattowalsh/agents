...
<Badge text="update-user-instructions" variant="note" /> <Badge text="817 words" variant="default" />

> Use when user wants to add, change, remove, or improve their global Claude Code instruction files (~/.claude/CLAUDE.md and ~/.claude/instructions/), or when they want a review of instruction quality and organization. Also use when user mentions "update my instructions", "improve my CLAUDE.md", "audit my global instructions", or "add a global rule".

**Install:** `npx skills add update-user-instructions -g`
**Use:** `/update-user-instructions`
...
````yaml title="SKILL.md"
---
name: update-user-instructions
description: >
  Use when user wants to add, change, remove, or improve their global
  Claude Code instruction files (~/.claude/CLAUDE.md and
  ~/.claude/instructions/), or when they want a review of instruction
  quality and organization. Also use when user mentions "update my
  instructions", "improve my CLAUDE.md", "audit my global instructions",
  or "add a global rule".
---

# Update User Instructions

Manage global Claude Code user instructions with best-practice guardrails.

## Architecture

- **Hub file**: `~/.claude/CLAUDE.md` — loaded into every session's system prompt. Contains cross-cutting rules and `@` imports.
- **Module files**: `~/.claude/instructions/<topic>.md` — domain/language-specific instructions imported via `@~/.claude/instructions/<topic>.md` syntax in the hub file.
- The `@` import syntax is how Claude Code natively loads additional files into context. Imports are resolved at session start and support recursive imports up to 5 hops deep.
- When new `@` imports are added to CLAUDE.md, Claude Code shows an approval dialog the first time. Inform the user to expect and approve this.

## Mode Dispatch

```
Has $ARGUMENTS?
├── Yes → Is it a concrete rule/change?
│         ├── Yes → Update Mode
│         └── No (meta-request: "optimize", "review", "clean up") → Audit & Optimize Mode
└── No  → Audit & Optimize Mode
```
...
Triggered when `$ARGUMENTS` contains a concrete instruction change.

### Steps

1. **Read current state**: Read `~/.claude/CLAUDE.md` and all `~/.claude/instructions/*.md` files.

2. **Parse intent**: Determine if `$ARGUMENTS` requests an add, modify, or remove.

3. **Classify target**:

   | Target | Placement |
   |--------|-----------|
   | Cross-cutting / behavioral rule | `~/.claude/CLAUDE.md` |
   | Language-specific (Python, JS, Go, etc.) | `~/.claude/instructions/<language>.md` |
   | Domain-specific (testing, git, security, etc.) | `~/.claude/instructions/<domain>.md` |

   If the target instruction file doesn't exist, create it AND add an `@` import to CLAUDE.md.

4. **Opportunistic optimization**: While editing, check if nearby CLAUDE.md content could be extracted to instruction files. If so, include extraction in the proposed changes.

5. **Consult best practices**: Read `references/best-practices.md` for composition guidance.

6. **Draft change** following best practices:
   - Imperative form, one rule per bullet
   - Concise — every line must earn context-window cost
   - Include brief `(eg ...)` rationale for non-obvious rules
   - No generic advice Claude already knows

7. **Preview**: Show diff(s) to user with brief rationale for placement and wording.

8. **Apply**: Edit file(s) upon user approval.

9. **Verify imports**: Ensure `~/.claude/CLAUDE.md` has an `@` import for every instruction file in `~/.claude/instructions/`.

### Edge Cases

- If `~/.claude/CLAUDE.md` doesn't exist → create from `references/templates.md` Template 1
- If `~/.claude/instructions/` doesn't exist → create the directory
- If user requests deletion → locate and remove the rule, clean up empty instruction files and orphaned `@` imports

---

## Audit & Optimize Mode

Triggered when `$ARGUMENTS` is empty or is a meta-request (e.g., "review", "optimize", "clean up").

### Steps

1. **Discover**: Read `~/.claude/CLAUDE.md` and glob `~/.claude/instructions/*.md`.

2. **Analyze CLAUDE.md for extraction candidates**: Classify every rule:

   | Keep in CLAUDE.md (hub) | Extract to instructions/ |
   |--------------------------|--------------------------|
   | Rules applying to EVERY session regardless of task | Rules specific to a language, framework, or tool |
   | Meta-rules about Claude's own behavior | Domain knowledge (eg "prefer fastapi for APIs") |
   | Critical overrides with emphasis markers | Detailed library/tool preferences |
   | `@` import declarations | Workflow instructions for specific contexts |
   | Rules about reviewing other docs (AGENTS.md, etc.) | Testing, security, git conventions |

   **Goal**: CLAUDE.md should be a concise router (~15-30 lines of rules + import block), not a monolith.

3. **Assess quality**: Evaluate hub + all instruction files against `references/quality-rubric.md`.

4. **Cross-reference**: Compare against `references/best-practices.md` for gaps and anti-patterns.

5. **Output summary report**:

   - **Overall health** (1-2 sentences)
   - **Structural optimization**: Proposed extraction plan with full diffs:
     ```
     EXTRACT: Lines about Python tooling → ~/.claude/instructions/python.md
     REPLACE IN CLAUDE.md: Remove extracted lines, add @~/.claude/instructions/python.md import
     ```
   - **Content improvements**: Top 3-5 most impactful recommendations, each with:
     - What to change and why
     - Concrete diff showing the proposed edit
   - **Missing coverage**: Suggested new instruction files based on detected gaps
   - **Import health**: Flag instruction files not `@`-imported in CLAUDE.md; flag broken `@` imports pointing to non-existent files
   - **FYI**: Mention that `~/.claude/rules/*.md` is the official auto-loaded location for user-level modular rules (no `@` imports needed), as an alternative the user may consider

6. **Apply selectively**: Ask user which recommendations to apply. Batch structural changes (extractions) separately from content changes. Always show full diffs before any edit.

---

## Rules

- **Safety first**: Always show full diffs before any edit; never modify files without explicit user approval
- Never remove existing instructions without explicit user approval
- Preserve the user's voice and wording style
- When adding new instruction files, always add corresponding `@` import in CLAUDE.md
- Keep `~/.claude/CLAUDE.md` lean: cross-cutting rules + `@` import block (~15-30 lines of rules ideal)
- Each instruction file should be focused on one topic
- File naming: lowercase, hyphenated (eg `python.md`, `code-review.md`, `git-workflow.md`)
- When extracting from CLAUDE.md, preserve the exact wording — just relocate it
- **Deduplication**: If a rule appears in both CLAUDE.md and an instruction file, flag it and propose removing the CLAUDE.md copy (keeping the modular version)
- **Broken import detection**: If an `@` import in CLAUDE.md points to a non-existent file, flag it for removal or file creation
