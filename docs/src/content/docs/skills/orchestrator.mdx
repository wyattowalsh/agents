...
> Parallel execution via subagent waves, teams, and pipelines. Use when 2+ independent actions need coordination. NOT for single-action tasks.
...
|------|-----------|----------|-------|
| **Team + nested waves (Pattern E)** | TeamCreate + subagent waves per teammate — up to ~50 agents total | 2+ independent streams — THE DEFAULT | opus (default) / gpt-5.4 xhigh (GitHub Copilot CLI) |
| **Subagent wave** | Task tool, parallel calls | 2+ actions, single domain, no coordination, no context pressure | opus (default) / gpt-5.4 xhigh (GitHub Copilot CLI) |
...
- GitHub Copilot CLI override: `gpt-5.4` with `xhigh` effort for every subagent, teammate, wave, and `/fleet` member.
