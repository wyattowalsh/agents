...
3. **DISPATCH** -- 2+ independent actions go as parallel Task calls in one message. Mix of independent and dependent means parallel first, dependent after. All dependent means single session.
...
|---------|------|----------|
| A | Parallel subagent wave | 2-6 independent subtasks in a session |
...
| E | Teams of subagent-using teammates | Very large tasks, nested parallelism |
...
| Model | Use for |
|-------|---------|
| `opus` | Default for ALL subagents. Implementation, analysis, reasoning, reviews. |
| `sonnet` | ONLY routine, mechanical tasks: straightforward file lookups, basic searches. |
| `haiku` | ONLY trivial, zero-risk tasks. Never for anything requiring judgment. |
