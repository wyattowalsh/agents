---
title: "nerdbot"
description: "Create and migrate Obsidian-native knowledge bases with layered raw/wiki structure, provenance, indexes, logs, and safe vault overhauls. Use for git-friendly KBs. NOT for docs sites or generic notes."
---

import { Badge, Tabs, TabItem, Card, CardGrid, LinkCard, Aside, Steps } from '@astrojs/starlight/components';

<Badge text="nerdbot" variant="note" /> <Badge text="3069 words" variant="default" /> <Badge text="MIT" variant="success" /> <Badge text="v0.2.0" variant="success" /> <Badge text="wyattowalsh" variant="caution" /> <Badge text="opus" variant="tip" /> <Badge text="Custom" variant="tip" />

> Create and migrate Obsidian-native knowledge bases with layered raw/wiki structure, provenance, indexes, logs, and safe vault overhauls. Use for git-friendly KBs. NOT for docs sites or generic notes.
...
**Install:**
```bash
npx skills add wyattowalsh/agents/skills/nerdbot -g
```

**Use:** `/nerdbot` `[create|ingest|enrich|audit|derive|improve|migrate] [topic|path]`
