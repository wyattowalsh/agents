...
- Repo instruction files and their generated managed-platform mirrors (`instructions/*.md`, scoped rules, and harness entrypoints)
- MCP, plugin, and OpenCode/TUI/DCP/ensemble config projections
- Skill roots and mirrors for the managed agent families
- Hook files, plannotator hooks, and various bundle manifests
- Grok, OpenCode, Codex, Claude Code, Cursor, and Crush repo/home configs

## Harness Surfaces & Sync Modes

`sync-manifest` feeds the exact managed agent families (Claude Code, Codex, Crush, Cursor, Grok Build, and OpenCode) plus separately classified MCP-only or hybrid clients such as Claude Desktop, ChatGPT, Cherry Studio, and LM Studio.
...
- [§2.7 Curated External Skills (Bucket A)](https://github.com/wyattowalsh/agents/blob/main/AGENTS.md#27-curated-external-skills-catalog--bucket-a): catalog authoring SSOT and generated index; `sync-manifest.json` tracks Bucket B registry surfaces under `config/`.
...
<CardGrid>
  <LinkCard title="AGENTS.md §2.7" href="https://github.com/wyattowalsh/agents/blob/main/AGENTS.md#27-curated-external-skills-catalog--bucket-a" description="Catalog authoring SSOT and Bucket B registry classification." />
...
````json title="config/sync-manifest.json"
{
  ...
  "managed": [
    ...
    {
      "path": "${REPO_ROOT}/docs/public/generated-registries/skills-catalog-index.json",
      "mode": "generated"
    },
    {
