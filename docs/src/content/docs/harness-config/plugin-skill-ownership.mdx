...
The candidate-corpus runtime overlay verifies 16 native plugin artifacts. Their
...
Active reviewed Codex plugin surfaces are `brooks-lint`, `codebase-recon`,
`commit-narrator`, `designer-skill`, `env-lint`, `prompt-to-asset`, `remotion`,
`roadmapsmith`, `secret-guard`, `universal-design-principles`, and `unslop`.
`axiom` and `hyperframes` are installed but disabled because their native
bundles contribute broad lifecycle hooks. `hol-guard-plugin`,
`papersflow-codex-plugin`, and `zotero-research-tools` also remain disabled at
their reviewed security or account boundary. Enabling any disabled plugin is a
separate activation decision; `wagents skills sync` does not make it active.
