...
| MCP schema tax | MCPHub `harness-safe` | `config/mcp-registry.json`; [MCPHub](/mcp/) § control plane |
...
MCP tool schemas are a major context tax. Prefer connecting harnesses to the **`harness-safe`** MCPHub group rather than projecting every server to every client.
...
<CardGrid>
  ...
  <LinkCard title="mcp-registry" href="/harness-config/mcp-registry/" description="MCP groups including harness-safe." />
