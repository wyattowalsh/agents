...
const worktreeConfigSchema = z.object({
	...
	sync: z
		.object({
			...
			/** Reserved for future use; sync exclusions are not currently enforced. */
			exclude: z.array(z.string()).default([]),
		})
		.default(() => ({ copyFiles: [], symlinkDirs: [], exclude: [] })),
	hooks: z
		.object({
			/** Explicitly allow repo-sourced hook commands to run */
			allowRepoCommands: z.boolean().default(false),
			/** Commands to run after worktree creation */
			postCreate: z.array(z.string()).default([]),
			/** Commands to run before worktree deletion */
			preDelete: z.array(z.string()).default([]),
		})
		.default(() => ({ allowRepoCommands: false, postCreate: [], preDelete: [] })),
})
...
async function validateOcxProfileAvailability(
	...
	try {
		...
		return Result.err(`profile check exited ${exitCode}`)
	} catch (error) {
		return Result.err("profile check failed before completion")
	}
}
...
async function copyDirIfExists(src: string, dest: string): Promise<boolean> {
	...
	await cp(src, dest, {
		...
		filter: async (source) => {
			...
			return realSource === realSrc || realSource.startsWith(realSrc + path.sep)
		},
	})
	return true
}

async function cleanupForkContext(projectId: string, sessionId: string, log: Logger): Promise<void> {
	...
	const workspaceDir = await resolveSafeStateSubdir(workspaceBase, projectId, sessionId)
	const delegationsDir = await resolveSafeStateSubdir(delegationsBase, sessionId)
	if (!workspaceDir || !delegationsDir) {
		log.warn("[worktree] Refusing to clean up fork context with unsafe state path")
		return
	}

	await rm(workspaceDir, { recursive: true, force: true }).catch((error) => {
		log.warn(`[worktree] Failed to clean up forked workspace context: ${error}`)
	})
	await rm(delegationsDir, { recursive: true, force: true }).catch((error) => {
		log.warn(`[worktree] Failed to clean up forked delegation context: ${error}`)
	})
}

function safeStateSubdir(baseDir: string, ...segments: string[]): string | null {
	if (!segments.every(isSafeStateSegment)) return null
...
function isSafeStateSegment(segment: string): boolean {
	return segment.length > 0 && segment === path.basename(segment) && segment !== "." && segment !== ".."
}

async function resolveSafeStateSubdir(baseDir: string, ...segments: string[]): Promise<string | null> {
	const lexicalPath = safeStateSubdir(baseDir, ...segments)
	...
	for (const segment of segments) {
		...
		if (currentInfo?.isSymbolicLink()) return null
	}

	const nearestExistingParent = await nearestExistingDir(path.dirname(lexicalPath))
	const realParent = await realpath(nearestExistingParent)
	if (realParent !== realBase && !realParent.startsWith(realBase + path.sep)) return null

	return lexicalPath
}

async function nearestExistingDir(dirPath: string): Promise<string> {
	let current = dirPath
...
async function forkWithContext(
	...
	try {
		...
		const srcWorkspaceDir = await resolveSafeStateSubdir(workspaceBase, projectId, rootSessionId)
		const srcDelegations = await resolveSafeStateSubdir(delegationsBase, rootSessionId)
		const destWorkspaceDir = await resolveSafeStateSubdir(workspaceBase, projectId, forkedSession.id)
		const destDelegationsDir = await resolveSafeStateSubdir(delegationsBase, forkedSession.id)
		if (!srcWorkspaceDir || !srcDelegations || !destWorkspaceDir || !destDelegationsDir) {
			throw new WorktreeError("Refusing to copy session context with unsafe state path", "forkWithContext")
		}
		...
		// Copy plan
		const srcPlan = path.join(srcWorkspaceDir, "plan.md")
				...
				client.app
					.log({
						body: {
							service: "worktree",
							level: "error",
							message: `forkWithContext: Failed to clean up workspace context: ${e}`,
						},
					})
					.catch(() => {})
			})
		}
		if (destDelegationsDir) {
				...
				client.app
					.log({
						body: {
							service: "worktree",
							level: "error",
							message: `forkWithContext: Failed to clean up delegation context: ${e}`,
						},
					})
					.catch(() => {})
			})
...
async function removeWorktree(
	...
	const result = await git(["worktree", "remove", worktreePath], repoRoot)
...
async function cleanupCreatedWorktree(
	...
	const result = await removeWorktree(repoRoot, worktreePath)
	if (result.ok) return

	log.warn(`[worktree] Worktree cleanup failed after setup error: ${result.error}`)
	log.warn(`[worktree] Manual cleanup may be required for: ${worktreePath}`)
}
...
function isSensitiveSyncPath(filePath: string): boolean {
	const normalized = filePath.replace(/\\/g, "/").replace(/^\.\//, "")
	const normalizedLower = normalized.toLowerCase()
	const segments   = normalized.split("/").filter(Boolean)
	const lowerSegments = normalizedLower.split("/").filter(Boolean)
	const basename   = lowerSegments.at(-1) ?? ""

	if (segments.length === 0 || normalizedLower === ".") return true
	...
	if (lowerSegments.some((segment) => sensitiveSegments.has(segment))) return true

	const normalizedPath = lowerSegments.join("/")
	if (normalizedPath === ".config/gcloud" || normalizedPath.startsWith(".config/gcloud/")) {
		return true
	}
	if (normalizedPath === ".docker/config.json" || normalizedPath.startsWith(".docker/config.json/")) {
		return true
	}
	if (normalizedPath === ".config/gh/hosts.yml") return true
	if (normalizedPath === ".config/opencode" || normalizedPath.startsWith(".config/opencode/")) {
		return true
	}
	if (normalizedPath === ".cargo/credentials.toml") return true
...
async function isSafeSyncSource(sourceDir: string, sourcePath: string, label: string, log: Logger): Promise<boolean> {
	const sourceInfo = await lstat(sourcePath).catch(() => null)
	if (!sourceInfo) return false
	if (sourceInfo.isSymbolicLink()) {
		log.warn(`[worktree] Refusing to sync symlink source: ${label}`)
		return false
	}

	const [realSourceDir, realSourcePath] = await Promise.all([realpath(sourceDir), realpath(sourcePath)])
	if (!realSourcePath.startsWith(realSourceDir + path.sep) && realSourcePath !== realSourceDir) {
		log.warn(`[worktree] Refusing to sync source outside repository: ${label}`)
		return false
	}
	return true
}

async function prepareSafeDestinationParent(
	targetDir: string,
	targetPath: string,
	label: string,
	log: Logger,
	...
	const relativeParent = path.relative(targetDir, targetParentDir)
	if (relativeParent.startsWith("..") || path.isAbsolute(relativeParent)) {
		log.warn(`[worktree] Refusing destination outside worktree: ${label}`)
		return false
	}
	...
	for (const segment of relativeParent.split(path.sep).filter(Boolean)) {
		...
		if (currentInfo?.isSymbolicLink()) {
			log.warn(`[worktree] Refusing destination with symlink parent: ${label}`)
			return false
		}
	}

	await mkdir(targetParentDir, { recursive: true })
	const [realTargetDir, realParentDir] = await Promise.all([realpath(targetDir), realpath(targetParentDir)])
	if (realParentDir !== realTargetDir && !realParentDir.startsWith(realTargetDir + path.sep)) {
		log.warn(`[worktree] Refusing destination escaping worktree: ${label}`)
		return false
	}
...
async function copyFiles(
	...
	for (const file of files) {
		...
		if (isSensitiveSyncPath(file)) {
			log.warn(`[worktree] Refusing to copy sensitive sync path: ${file}`)
			continue
		}
		...
		try {
			if (!(await isSafeSyncSource(sourceDir, sourcePath, file, log))) continue
			if (!(await prepareSafeDestinationParent(targetDir, targetPath, file, log))) continue
			const targetInfo = await lstat(targetPath).catch(() => null)
			if (targetInfo?.isSymbolicLink()) {
				log.warn(`[worktree] Refusing to overwrite symlink destination: ${file}`)
				continue
			}
...
async function symlinkDirs(
	...
	for (const dir of dirs) {
		...
		if (isSensitiveSyncPath(dir)) {
			log.warn(`[worktree] Refusing to symlink sensitive sync path: ${dir}`)
			continue
		}
		...
		try {
			if (!(await isSafeSyncSource(sourceDir, sourcePath, dir, log))) continue
			if (!(await prepareSafeDestinationParent(targetDir, targetPath, dir, log))) continue
...
async function runHooks(cwd: string, commands: string[], log: Logger): Promise<void> {
	for (const [index, command] of commands.entries()) {
		const hookNumber = index + 1
		log.info(`[worktree] Running hook ${hookNumber}/${commands.length}`)
		try {
			...
			if (result.exitCode !== 0) {
				log.warn(
					`[worktree] Hook ${hookNumber}/${commands.length} failed with exit ${result.exitCode}; inspect the worktree shell for details`,
				)
			}
		} catch (error) {
			log.warn(`[worktree] Hook ${hookNumber}/${commands.length} failed before completion: ${error}`)
		}
	}
}

function warnSkippedHooks(hookName: string, commands: string[], log: Logger): void {
	if (commands.length === 0) return
	log.warn(
		`[worktree] Skipping ${commands.length} ${hookName} hook(s); set hooks.allowRepoCommands=true to run repo-sourced commands`,
	)
}
...
async function loadWorktreeConfig(directory: string, log: Logger): Promise<WorktreeConfig> {
  ...
  "sync": {
    ...
    // Avoid copying secrets such as .env*, key files, and credential stores.
    // Example: ["dev.sqlite"]
    "copyFiles": [],
  ...
  "hooks": {
    // Repo-sourced hooks are disabled by default because they run shell commands.
...
const WorktreePlugin: Plugin = async (ctx) => {
	...
	return {
		tool: {
			worktree_create: tool({
				...
				async execute(args, toolCtx) {
					...
					let forkedSessionIdForCleanup: string | null = null
					...
					try {
						...
						// Run postCreate hooks only after explicit opt-in.
						if (worktreeConfig.hooks.allowRepoCommands && worktreeConfig.hooks.postCreate.length > 0) {
							await runHooks(worktreePath, worktreeConfig.hooks.postCreate, log)
						} else {
							warnSkippedHooks("postCreate", worktreeConfig.hooks.postCreate, log)
						}
						...
						forkedSessionIdForCleanup = forkedSession.id
						...
						if (!terminalResult.success) {
							await cleanupForkContext(projectId, forkedSession.id, log)
							await cleanupCreatedWorktree(directory, worktreePath, log)
							return `❌ Failed to launch worktree terminal: ${terminalResult.error ?? "unknown error"}\nWorktree cleanup was attempted at ${worktreePath}.`
						}

						return `Worktree created at ${worktreePath}\n\nA new terminal has been opened with OpenCode.`
					} catch (error) {
						if (projectIdForCleanup && forkedSessionIdForCleanup) {
							await cleanupForkContext(projectIdForCleanup, forkedSessionIdForCleanup, log)
						}
						await cleanupCreatedWorktree(directory, worktreePath, log)
						return `❌ Failed to finish worktree setup: ${error instanceof Error ? error.message : String(error)}\nWorktree cleanup was attempted at ${worktreePath}.`
					}
				},
			}),
			...
			worktree_delete: tool({
				description:
					"Delete the current worktree and clean up. Cleanup aborts if uncommitted changes are present.",
				args: {
					reason: tool.schema
						.string()
				...
				async execute(_args, toolCtx) {
					...
					// Set pending delete for this session.idle event only.
					setPendingDelete(
						database,
		...
		event: async ({ event }: { event: Event }): Promise<void> => {
			...
			// Handle pending delete for this session only.
			const pendingDelete = getPendingDelete(database, event.properties.sessionID)
			if (pendingDelete) {
				const { path: worktreePath } = pendingDelete

				const statusResult = await git(["status", "--porcelain"], worktreePath)
				if (!statusResult.ok) {
					log.warn(`[worktree] cleanup aborted; git status failed: ${statusResult.error}`)
					return
				}
				if (statusResult.value.length > 0) {
					log.warn("[worktree] cleanup aborted; commit or discard worktree changes before deleting")
					return
				}

				// Run preDelete hooks only after the worktree is known clean.
				const config = await loadWorktreeConfig(directory, log)
				if (config.hooks.allowRepoCommands && config.hooks.preDelete.length > 0) {
					await runHooks(worktreePath, config.hooks.preDelete, log)
				} else {
					warnSkippedHooks("preDelete", config.hooks.preDelete, log)
				}

				const postHookStatusResult = await git(["status", "--porcelain"], worktreePath)
				if (!postHookStatusResult.ok) {
					log.warn(`[worktree] cleanup aborted; post-hook git status failed: ${postHookStatusResult.error}`)
					return
				}
				if (postHookStatusResult.value.length > 0) {
					log.warn("[worktree] cleanup aborted; preDelete hooks changed the worktree")
					return
				}
				...
				clearPendingDeleteForSession(database, event.properties.sessionID)

				// Remove session from database
				removeSessionById(database, event.properties.sessionID)
			}
		},
	}
}
