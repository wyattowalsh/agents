...
export async function initStateDb(projectRoot: string): Promise<Database> {
	...
	ensurePendingOperationsSchema(db)

	return db
}

function ensurePendingOperationsSchema(db: Database): void {
	...
	if (idColumn?.type?.toUpperCase() !== "TEXT") {
		rebuildPendingOperationsTable(db, tableInfo)
	}

	normalizePendingDeleteIds(db)
}

function rebuildPendingOperationsTable(
	db: Database,
	tableInfo: Array<{ name?: string; type?: string }>,
): void {
	const migrate = db.transaction(() => {
		const columns = new Set(tableInfo.map((column) => column.name).filter(Boolean))
		const selectLegacyColumn = (column: string, alias = column): string => {
			return columns.has(column) ? `${column} as ${alias}` : `NULL as ${alias}`
		}
		const legacyRows = db.prepare(`
			SELECT
				${selectLegacyColumn("type")},
				${selectLegacyColumn("branch")},
				${selectLegacyColumn("path")},
				${selectLegacyColumn("session_id", "sessionId")}
		...
		restorePendingRows(db, legacyRows)
	})
	migrate()
}

function restorePendingRows(db: Database, rows: Array<Record<string, string | null>>): void {
	const restoreStmt = db.prepare(`
		INSERT OR REPLACE INTO pending_operations (id, type, branch, path, session_id)
		VALUES ($id, $type, $branch, $path, $sessionId)
	`)

	for (const row of rows) {
		if (row.type === "spawn") {
			const parsed = pendingSpawnSchema.safeParse({
				branch: row.branch,
				path: row.path,
		...
		if (row.type === "delete") {
			const parsed = pendingDeleteSchema.safeParse({
				branch: row.branch,
				path: row.path,
				sessionId: row.sessionId,
			})
			...
			restoreStmt.run({
				$id: deleteOperationId(parsed.data.sessionId),
				$type: "delete",
				$branch: parsed.data.branch,
				$path: parsed.data.path,
				$sessionId: parsed.data.sessionId,
...
function normalizePendingDeleteIds(db: Database): void {
	const rows = db.prepare(`
		SELECT id, type, branch, path, session_id as sessionId
		FROM pending_operations
		WHERE type = 'delete'
	`).all() as Array<Record<string, string | null>>
	...
	const migrate = db.transaction(() => {
		for (const row of rows) {
			...
			const expectedId = deleteOperationId(parsed.data.sessionId)
			if (row.id === expectedId) continue

			db.prepare(`DELETE FROM pending_operations WHERE id = $id`).run({ $id: row.id })
			db.prepare(`
				INSERT OR REPLACE INTO pending_operations (id, type, branch, path, session_id)
				VALUES ($id, 'delete', $branch, $path, $sessionId)
			`).run({
				$id: expectedId,
				$branch: parsed.data.branch,
				$path: parsed.data.path,
...
export function setPendingSpawn(db: Database, spawn: PendingSpawn, client?: OpencodeClient): void {
	...
	const existingDelete = getPendingDelete(db, parsed.sessionId)
	...
	const stmt = db.prepare(`
		INSERT OR REPLACE INTO pending_operations (id, type, branch, path, session_id)
		VALUES ('spawn', 'spawn', $branch, $path, $sessionId)
	`)
...
export function clearPendingSpawn(db: Database): void {
	const stmt = db.prepare(`DELETE FROM pending_operations WHERE id = 'spawn' AND type = 'spawn'`)
	stmt.run()
}
...
/**
 * Set a pending delete operation for a specific session.
 *
 * @param db - Database instance from initStateDb
...
export function setPendingDelete(db: Database, del: PendingDelete, client?: OpencodeClient): void {
	...
	// Atomic: replace this session's pending delete without affecting other sessions.
	const stmt = db.prepare(`
		INSERT OR REPLACE INTO pending_operations (id, type, branch, path, session_id)
		VALUES ($id, 'delete', $branch, $path, $sessionId)
	`)
	...
	stmt.run({
		$id: deleteOperationId(parsed.sessionId),
		$branch: parsed.branch,
		$path: parsed.path,
		$sessionId: parsed.sessionId,
	})
 ...
 */
export function getPendingDelete(db: Database, sessionId: string): PendingDelete | null {
	const parsedSessionId = z.string().min(1).parse(sessionId)
	const stmt = db.prepare(`
		SELECT type, branch, path, session_id AS sessionId
		FROM pending_operations
		WHERE id = $id AND type = 'delete'
	`)

	const row = stmt.get({ $id: deleteOperationId(parsedSessionId) }) as Record<string, string> | null
	if (!row) return null
...
export function clearPendingDelete(db: Database): void {
	const stmt = db.prepare(`DELETE FROM pending_operations WHERE type = 'delete'`)
	stmt.run()
}

export function clearPendingDeleteForSession(db: Database, sessionId: string): void {
	const parsedSessionId = z.string().min(1).parse(sessionId)
	const stmt = db.prepare(`DELETE FROM pending_operations WHERE id = $id AND type = 'delete'`)
	stmt.run({ $id: deleteOperationId(parsedSessionId) })
}

function deleteOperationId(sessionId: string): string {
