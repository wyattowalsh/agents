...
| Grok Build         | `AGENTS.md` → `@instructions/global.md`                        | `instructions/grok-global.md`, `config/grok-config.toml`, `~/.grok/config.toml`, `.grok/config.toml`; MCP via sync; `/grok-delegate` for cross-harness native CLI task graphs; delegation pre-flight `/grok-delegate preflight` or `bash skills/grok-delegate/scripts/preflight.sh` (bundled doctor; harness maintainer doctor: `wagents grok doctor --format json`); Plannotator via CLI + skills + `config/grok-plannotator-hooks.json` (not OpenCode plugin); `wagents grok plannotator install` |
