...
| Grok Build         | `AGENTS.md` → `@instructions/global.md`                        | `instructions/grok-global.md`, `config/grok-config.toml`, `~/.grok/config.toml`, `.grok/config.toml`; MCP via sync; `/grok-delegate` for cross-harness native CLI task graphs; pre-flight `wagents grok doctor --format json`; Plannotator via CLI + skills + `config/grok-plannotator-hooks.json` (not OpenCode plugin); `wagents grok plannotator install` |
