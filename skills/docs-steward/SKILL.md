...
name: docs-steward
description: >-
  Maintains docs across Starlight, Docusaurus, Fumadocs, Sphinx themes, and
  MkDocs. Detects stacks, syncs artifacts, runs health checks, refreshes latest
  versions, and plans migrations. Use when docs frameworks or pipelines
  change. NOT for backend/API work or creating skills, agents, or MCP servers.
argument-hint: "[auto|init [framework]|init-sync [framework]|sync|enhance|maintain|research|framework <name> [sync|enhance|maintain|research|migrate]|matrix|migrate <from> -> <to>]"
license: MIT
model: opus
metadata:
  author: wyattowalsh
...
**In scope:** Docs framework operations, non-destructive docs bootstrap, version refresh, build/health diagnostics, quality enhancement, docs migrations.
