...
3. Run `uv run python scripts/diff-summarizer.py` on the diff output
...
3. Run `uv run python scripts/commit-parser.py` on git log output
4. Run `git diff <base>...HEAD --stat` for change statistics
5. Run `uv run python scripts/diff-summarizer.py` on the diff stat
