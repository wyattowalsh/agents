...
4. **Treat agreement across engines as confidence signal** — if brave-search and duckduckgo-search return the same claim from different source URLs, that is stronger than two results from the same engine
...
|------|----------------|----------|
| brave-search + duckduckgo-search | High (different indices) | General verification |
| brave-search + exa | High (keyword vs semantic) | Technical topics |
| exa + tavily | Medium (both AI-optimized) | Deep research queries |
| g-search + duckduckgo-search | High (Google vs Bing-based) | Broad coverage |
...
| brave-search | 0.5 max | duckduckgo-search or exa for web search | Reduced index coverage |
| All web search (brave + duckduckgo-search + exa + g-search + tavily) | 0.4 max | context7/deepwiki for library-specific only | Cannot verify current events |
