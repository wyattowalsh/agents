...
_COMPARISON_PATTERNS = re.compile(r"\b(vs\.?|versus|compared\s+to|better\s+than|worse\s+than)\b", re.IGNORECASE)
_COMPARISON_OR_PATTERN = re.compile(r"\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+or\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b")
