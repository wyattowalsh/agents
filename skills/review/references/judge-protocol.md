...
Pipe raw reviewer output through skills/review/scripts/finding-formatter.py:

```bash
cat reviewer-output.txt | uv run python skills/review/scripts/finding-formatter.py --mode session  # or --mode audit
```

Judge assigns each surviving finding a canonical ID such as `RV-{S|A|PR|SRC}-{seq:03d}` after normalization and dedupe. Worker-provided IDs are kept as `source_id` or `local_id`.

Example: `{"id": "RV-S-001", "priority": "P1", "location": "src/auth.py:45", "confidence": 0.85, "description": "Missing token expiry validation", "evidence": "RFC 6749 Section 5.1"}`
...
Example: RV-S-003 at `routes.py:112`, RV-S-007 at `routes.py:118` (within 10 lines),
RV-S-012 at `handlers.py:45` (same pattern) — all cluster into `C-001: missing input validation`.
...
Example: RV-S-003 (0.9, cites OWASP) and RV-S-007 (0.7, cites library docs) — keep
RV-S-003, merge the library doc citation into its evidence field.
...
```bash
cat findings.json | uv run python skills/review/scripts/learnings-store.py check --project [project-slug]
```

Findings matching a stored false-positive learning have their confidence reduced by 0.3.
The default command is read-only. Use `--record-match` only after a review mode
is allowed to update local review state and the match is relevant to the
current review context.

See skills/review/scripts/learnings-store.py for matching criteria (file path + category/keyword).
...
Example: `RV-S-005 [P0, 0.55]` — reported as unconfirmed (P0 exception). `RV-S-014 [P2, 0.20]` — discarded to audit trail.
...
- **Irreconcilable**: present both, mark `CONFLICT — see also RV-{id}`, defer to reviewer.

Example: RV-S-008 ("1:1 wrapper, 4 methods, 0 bypass callers" — concrete) vs RV-S-011 ("needed for future auth hooks" — heuristic). RV-S-008 wins.
...
| Interaction | Action |
|-------------|--------|
| **Conflict** — fixing A worsens B | Annotate both: "Conflicts with RV-{id}" |
| **Subsumption** — fixing A fixes B | Keep A, annotate B: "Resolved by RV-{id}" |
| **Independent** | No annotation |

Group related fixes into implementation batches (`Batch: B-{n}`).

Example: RV-S-005 (race condition) and RV-S-009 (missing lock) — `Batch B-1`: implement
...
Example: `RV-S-005 [P0, 0.85, system-wide] = 10 x 0.85 x 5 = 42.5 → Must Fix`
...
MUST FIX:
1. RV-S-005 [P0, score 42.5] Race condition in session store
   Level: Correctness | Confidence: 0.85 | Effort: M
   Evidence: [citation] | Impact: [consequence] | Fix: [approach]
   Batch: B-1 (implement with RV-S-009)

SHOULD FIX:
2. RV-S-003 [P1, score 13.5] SYSTEMIC — missing input validation
...
CONSIDER:
3. RV-S-009 [P1, score 7.5] Missing session lock
   Level: Correctness | Confidence: 0.75 | Effort: S | Batch: B-1

UNCONFIRMED:
4. RV-S-019 [P0] Potential memory leak — P0 exception (confidence: 0.45)

DISCARDED (audit trail):
- RV-S-014 [P2, 0.20] Possible over-fetching — insufficient evidence

CONFLICTS: RV-S-008 vs RV-S-011 (resolved: RV-S-008 wins)
BATCHES: B-1 [RV-S-005, RV-S-009] — session locking (implement together)
======================================
```
