...
| Outcome | Use When |
|---|---|
| `install-now-after-trust-gate` | Decision label only: trust gate passes and the skill fills a clear gap; do not execute a live install unless the user separately requests it |
| `inspect-then-install` | Decision label only: candidate is useful but scripts/provenance/credential behavior needs review before any live install |
