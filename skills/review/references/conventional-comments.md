...
**[RV-{S|A|PR|R|SRC|SIM}-{NNN}]** [file:start-end] | Level: <level> | Confidence: <score>
...
- **Traditional format** (default): Use for session reviews (`/review` with session changes) and full audits (`/review audit`). This is the standard output format from references/output-formats.md.
- **Conventional Comments format**: Use when:
  - `--format conventional` flag is explicitly set
  - Output destination is a PR review (`/review <PR#>`)
  - Running in CI pipeline (references/ci-integration.md)
- **Both formats together**: When reviewing a PR, produce the full traditional report in the conversation. Post individual findings as Conventional Comments only when the user explicitly requests publishing comments; otherwise provide dry-run comment bodies.

Cross-references: references/output-formats.md (traditional format), references/ci-integration.md (CI publishing).
