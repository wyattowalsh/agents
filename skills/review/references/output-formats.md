...
P0 - MUST FIX:
1. RV-S-001 [file:start-end] Missing null check
   Level: Correctness
   Confidence: 0.95
...
P1 - SHOULD FIX:
2. RV-S-002 [file:start-end] Query in loop
   Level: Efficiency
   Confidence: 0.85
...
S0 - SHOULD SIMPLIFY:
3. RV-S-003 [module] Service layer wraps repository 1:1
   Level: Design
   Confidence: 0.90
...
IMPROVEMENTS MADE:
- #1 (RV-S-001): Added null check [x]
- #2 (RV-S-002): Refactored to batch [x]
...
CRITICAL (P0/S0):
1. RV-A-001 [file:start-end or module] [category] description
...
Use when comparing against a previous review (via `skills/review/scripts/review-store.py diff`).
...
1. RV-S-001 [file:start-end] [description]
   Level: [level] | Confidence: [score] | Reasoning: [WHY] | Evidence: [citation]

RESOLVED (in baseline, not in current):
- RV-S-003 [was: file:line] [description] — resolved by [commit/change]

RECURRING (present in both):
- RV-S-002 [file:line] [description] — unchanged since [baseline date]
...
**Finding ID format:** `RV-{mode}-{seq}` where mode is `S` (session), `A`
(audit), `PR`, `R` (range), `SRC` (source/provenance), or `SIM`
(simplification). `seq` is zero-padded 3-digit (for example, `RV-S-001`,
`RV-A-015`, `RV-PR-003`, `RV-R-004`, `RV-SRC-005`, `RV-SIM-006`).
See `skills/review/scripts/finding-formatter.py` for programmatic ID generation.
...
- **Conventional Comments**: `--format conventional` flag, PR reviews (`/review <PR#>`), CI annotations

**Compact example:**
```
issue (blocking): Race condition in concurrent session access

**[RV-S-005]** [src/session.py:45-52] | Level: Correctness | Confidence: 0.85
...
- **SARIF v2.1**: `skills/review/scripts/finding-formatter.py --format sarif` — see references/sarif-output.md
- **JSON Schema**: `skills/review/scripts/finding-formatter.py --format json-schema` — pre-Judge normalized finding objects, not the full final finding contract
