...
```bash
uv run python skills/review/scripts/project-scanner.py [path]
```
...
Spawn three parallel reviewer subagents, one per dimension, in a single message. Collect all results before proceeding to risk stratification.
...
Run `skills/review/scripts/project-scanner.py` which includes dependency graph output, or extract manually.
...
Cross-references: skills/review/scripts/project-scanner.py, references/team-templates.md.
