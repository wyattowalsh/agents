...
Before spawning many subagents, the lead produces a shard map. Do not dispatch a large review until every shard has an owner and every owner has a bounded artifact contract.

| Field | Requirement |
| --- | --- |
| `lane_id` | Stable lane label such as `L-backend`, `L-docs`, `L-source-provenance`, or `L-a11y` |
| `shard_id` | Stable shard label within the lane, such as `L-backend:S03` |
| `owner` | Reviewer or subagent role responsible for the shard |
| `scope` | Exact files, directories, PR hunks, source paths, URLs, or command-output evidence owned by the shard |
| `lens` | Primary lens plus any cross-cutting specialist lens |
| `coverage` | What must be inspected and what is explicitly out of scope |
| `status` | `planned`, `in-progress`, `complete`, `blocked`, or `skipped-with-reason` |

Shard rules:

- File/source ownership is non-overlapping by default.
- Cross-cutting specialists may overlap, but their artifact must name the primary shard(s) they inspected.
- Same-file edit work is never parallel. Read-only review can overlap only when the lenses are intentionally independent.
- Worker findings use local IDs. Judge assigns canonical `RV-*` IDs after all shard artifacts return.
...
**Diminishing returns:** More than 6 parallel reviewers against the same lane/shard at the same time shows diminishing quality. For broader audits, preserve many-subagent coverage by splitting ownership into additional bounded shards and waves.
...
```
Pass A: Shard-Local Scan (parallel reviewer subagents)
  → Each subagent scans only its assigned shard
  → Shard findings determine priority routing to Pass B
  → Output: shard artifact with preliminary findings, non-findings, coverage, and blockers

Pass B: Deep Dive (reviewer subagents, 1 per HIGH-risk flagged file)
...
Pass C: Research Validate (mixed models, batched by validation type)
  → Group findings by type, dispatch per references/research-validation.md
  → Assign confidence scores per the Confidence Scoring Rubric
  → Output: validated findings with confidence scores and citations
```

Teammates report findings with: local ID, lane ID, shard ID, priority, confidence score, evidence, citation, effort estimate, and coverage notes. Judge assigns canonical RV IDs.
...
>
> **Pass A — Shard-Local Scan (reviewer subagents):**
>
> Dispatch parallel reviewer subagents against assigned shards only:
>
> | Subagent | Ownership | Rationale |
> |----------|-----------|-----------|
> | Pass A-1 | Shard S1 | Bounded source ownership |
> | Pass A-2 | Shard S2 | Parallel coverage without duplicate work |
> | Pass A-3 | Shard S3 | Parallel coverage without same-file write conflicts |
>
> **Coverage routing:** HIGH-risk shard findings get priority routing to Pass B.
> LOW-risk shards with no findings still return a coverage/non-findings artifact.
>
...
>
> **Pass B — Deep Dive:** Spawn parallel reviewer subagents (1 per HIGH-risk flagged file). Full analysis with at least 2 creative lenses (references/review-lenses.md). Check context-dependent items from references/checklists.md. For security-sensitive code, Adversary lens is mandatory.
>
> **Pass C — Research Validate:** Collect all findings from Passes A and B. Group by validation type and dispatch per references/research-validation.md. Assign confidence scores (0.0-1.0) using the Confidence Scoring Rubric.
>
> Also check: AI code smells (slopsquatting, hallucinated APIs, model-specific patterns, test-implementation mismatch).
>
> For each finding, include: local ID, lane ID, shard ID, priority (P0-P2/S0-S2), level, confidence score, evidence with citation, effort estimate (S/M/L), and coverage notes.
>
...
>
> - **Pass A:** shard-local scan by assigned ownership; no subagent scans files outside its shard unless assigned a cross-cutting lens
> - **Pass B:** Deep dive HIGH-risk flagged files (reviewer subagents, 1 per file)
> - **Pass C:** Research validate findings (batched per references/research-validation.md)
>
...
>
> Assign local IDs, confidence scores (0.0-1.0), and effort estimates (S/M/L). Judge assigns final RV IDs.
>
...
> Assign local specialist IDs only, such as `SEC-local-001`; Judge assigns canonical `RV-*` IDs after all artifacts are normalized and deduped.
> Return a specialist artifact with local ID, lane ID, inspected shard ID(s), exact files/sources inspected, findings, non-findings, blockers, coverage notes, confidence, and evidence. For cross-cutting overlap, name the primary shard(s) inspected.
...
> Assign local specialist IDs only, such as `OBS-local-001`; Judge assigns canonical `RV-*` IDs after all artifacts are normalized and deduped.
> Return a specialist artifact with local ID, lane ID, inspected shard ID(s), exact files/sources inspected, findings, non-findings, blockers, coverage notes, confidence, and evidence. For cross-cutting overlap, name the primary shard(s) inspected.
...
Pass A (reviewer): For each changed/reviewed file, list newly added functions, classes, and logic blocks. Flag duplication candidates by name/behavior.

Pass B (reviewer): For each Pass A candidate, Grep `utils/`, `lib/`, `common/`, `helpers/`, shared modules, adjacent files for matching behavior. Build citation table: new location → existing equivalent (file:line). Also Grep for stringly-typed patterns (raw strings vs existing constants/enums) and compare signatures for parameter sprawl.

Pass C (reviewer): Read cited lines to verify behavioral equivalence. Confidence: 0.9 exact match, 0.7 near match, discard < 0.5. Evidence = Grep result. Format as `[new:file:line] → existing [file:line]`.
...
Pass B (reviewer): Deep dive flagged items. Read source to confirm coverage gaps are real (not covered by integration/e2e tests). Verify mock usage against real interface signatures.

Pass C (reviewer): Research validate flagged patterns (WebSearch for flakiness patterns, Context7 for test framework best practices). Assign confidence. Format findings with citation anchors. Untested public functions = P2 minimum.
...
Pass A (reviewer): Read the auto-detected spec context passed from triage (PR description, linked issues, SPEC.md, docs/, commit bodies).

Pass B (reviewer): Compare each stated requirement against the implementation. Note gaps where requirements are not implemented, and divergences where implementation differs from requirements.

Pass C (reviewer): Research-validate disputed interpretations (WebSearch, Context7). Format divergences as P2 findings with dual citation anchors (spec text location + non-conforming code location).
...
Cross-references: references/triage-protocol.md (risk classification, specialist triggers), references/judge-protocol.md (reconciliation of findings across reviewers), references/research-validation.md (confidence scoring, batch routing), references/output-formats.md (finding format, report structure).
