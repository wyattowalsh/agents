...
- Model: research reviewer (needs judgment to compare usage against docs)
...
- Model: research reviewer (needs nuance for security assessment)
...
- Model: reviewer (structured data extraction)
...
- Model: reviewer (structured registry lookups)
...
- Model: reviewer (structured extraction from DeepWiki responses)
...
- Model: reviewer (fast tool calls, structured verification)
...
- Model: research reviewer (needs judgment to evaluate test quality)

## Parallelism Strategy

- Spawn ALL research subagents for a review pass in a single message
- Use reviewer for simple lookups (doc extraction, version checks)
- Use research reviewer for nuanced validation (security, architecture decisions)
...
- Use reviewer for lens-based analysis (requires creative reasoning); use reviewer for slopsquatting detection (structured lookups)
...
| ---------- | ------------------------------------ | ------------------------------------------------------------------------------------------- |
| HIGH       | Validate with 2+ independent sources | Context7 + WebSearch (both required). If sources disagree, escalate to reviewer-level analysis. |
...
1. Slopsquatting detection (security-critical, fast, reviewer)
2. HIGH-risk findings (require 2+ sources, research reviewer)
3. MEDIUM-risk findings (require 1 source, research reviewer or reviewer)
