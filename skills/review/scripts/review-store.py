#!/usr/bin/env python3
"""Persist and compare review results across sessions.

Stores review JSON in the active agent's reviews state directory.
...
STATE_DIR = get_agent_dir("reviews")

# Finding ID pattern: RV-S-001, RV-A-015, RV-SRC-002, etc.
FINDING_ID_RE = re.compile(r"^RV-[A-Z]+-\d{3}$")
LOCATION_LINE_RE = re.compile(r"^(?P<start>\d+)(?:-(?P<end>\d+))?$")
...
def review_filename(review_date: str, project: str, mode: str, run_id: str | None = None) -> str:
    """Build the canonical filename for a review."""
    suffix = f"-{run_id}" if run_id else ""
    return f"{review_date}-{slugify(project)}-{mode}{suffix}.json"
...
REVIEW_FILENAME_RE = re.compile(
    r"^\d{4}-\d{2}-\d{2}-(.+)-(session|audit|pr|range|source|simplify|scoped)(?:-[A-Za-z0-9_.-]+)?\.json$"
)
...
def _finding_key(f: dict[str, Any]) -> str:
    ...
    Prefer content/location fingerprints over display IDs. Review IDs are
    often reassigned on each run, so using them first causes false recurrence.
    """
    loc = f.get("location", "")
    file_part = loc.rsplit(":", 1)[0] if ":" in loc else loc
    cat = _normalize_text(f.get("category", ""))
    desc = _normalize_text(f.get("description") or f.get("finding", ""))
    if file_part or cat or desc:
        return f"fp:{file_part}|{cat}|{desc[:160]}"
    fid = f.get("id", "")
...
def _parse_location(location: str) -> tuple[str, tuple[int, int] | None]:
    """Parse `file:start[-end]` while preserving invalid anchors as exact paths."""
    if ":" not in location:
        return location, None

    file_part, line_part = location.rsplit(":", 1)
    match = LOCATION_LINE_RE.match(line_part)
    if not match:
        return location, None
...
def _ranges_near(range_a: tuple[int, int], range_b: tuple[int, int]) -> bool:
    """Return true when two ranges overlap or their nearest edges are close."""
...
def _location_proximity(loc_a: str, loc_b: str) -> bool:
    ...
    if not loc_a or not loc_b:
        return False
    file_a, range_a = _parse_location(loc_a)
    file_b, range_b = _parse_location(loc_b)
    if file_a != file_b:
        return False
    if range_a is None or range_b is None:
        # Same file but at least one side has no canonical range.
        return True
    return _ranges_near(range_a, range_b)
...
def diff_findings(
    ...
    Matching strategy:
    1. Match by stable finding fingerprint (file/category/description).
...
def cmd_save(args: argparse.Namespace) -> None:
    ...
    run_id = args.run_id or datetime.now().strftime("%H%M%S-%f")
    ...
    state = ensure_state_dir()
    filename = review_filename(today, project_slug, args.mode, run_id)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Persist and compare review results.",
    ...
    sp_save.add_argument(
        "--mode",
        choices=["session", "scoped", "pr", "range", "audit", "simplify", "source"],
        default="session",
        help="Review mode.",
    )
    ...
    sp_save.add_argument("--run-id", default=None, help="Override run ID used in the saved filename.")
