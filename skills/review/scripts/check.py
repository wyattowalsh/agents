#!/usr/bin/env python3
"""Validate the review skill, evals, package, and audit score."""
...
def _audit_script() -> Path | None:
    if _portable_ci():
        print("audit skipped: PORTABLE_CI mode (repo-only audit omitted)", file=sys.stderr)
        return None
    audit_script = SKILL_DIR.parent / "skill-creator" / "scripts" / "audit.py"
    if audit_script.is_file():
        return audit_script
    print("audit skipped: skill-creator not available (repo-only step)", file=sys.stderr)
    return None
