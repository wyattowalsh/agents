...
MANIFEST_MAP: dict[str, tuple[str, str, str | None]] = {
    "pyproject.toml": ("python", "pyproject.toml", "uv"),
    "setup.py": ("python", "setup.py", "pip"),
    "requirements.txt": ("python", "requirements.txt", "pip"),
    "package.json": ("javascript", "package.json", None),
...
HIGH_RISK_RE = re.compile(
    ...
    r"admin|permission|rbac|acl|security|migration|schema)",
    re.I,
)
SKIP = {
    ".git",
        ...
        for m in re.finditer(r"(?:dependencies|requires)\s*=\s*\[([^\]]*)\]", txt):
            for dm in re.finditer(r'"([a-zA-Z0-9_][a-zA-Z0-9_.-]*?)(\[[^\]]*\])?\s*([><=!~][^"]*)?"\s*', m.group(1)):
                deps.append({
                    ...
                    "version": (dm.group(3) or "").strip() or "*",
...
def scan(root: Path) -> dict:
    ...
    hot_files = [f for f, c in hc.most_common(20) if c >= 3]
    ...
    if n_contrib >= 3:
        for hf in hot_files[:10]:
            ...
            if len(fb.splitlines()) >= max(2, n_contrib // 2):
                high_blame.append(hf)
    ...
    if (
        ...
        and "pytest" in (root / "pyproject.toml").read_text(errors="replace")
    ):
        tfw = "pytest"
    ...
    return {
        ...
        "git_stats": {"hot_files": hot_files, "high_blame_density": high_blame},
        "risk_summary": rs,
    }
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Deterministic project pre-scan: detect type, compute metrics, stratify risk. Outputs JSON to stdout."
        ),
    )
    parser.add_argument(
        "path",
...
if __name__ == "__main__":
    ...
    if not target.is_dir():
        ...
        sys.exit(1)
    json.dump(scan(target), sys.stdout, indent=2)
    print()
