...
RISK_PATTERNS: list[tuple[str, str, str]] = [
    ...
    (
        "credential",
        rf"(?i:(?:api[_-]?key|token|secret|password|credential|private[_-]?key)\s*[:=]\s*['\"]?{SECRET_VALUE}|"
        r"\.env(?:\b|$))",
        "high",
    ),
...
def redact_excerpt(line: str) -> str:
    """Redact secret-looking assignments while preserving review context."""
    redacted = re.sub(
        rf"(?i:((?:api[_-]?key|token|secret|password|credential|private[_-]?key)\s*[:=]\s*['\"]?){SECRET_VALUE})",
        r"\1[REDACTED]",
        line,
        ...
        for kind, pattern, severity in RISK_PATTERNS:
            if re.search(pattern, line, flags=re.IGNORECASE):
                findings.append({
                    ...
                    "line": line_no,
                    "excerpt": redact_excerpt(line) if kind == "credential" else line.strip()[:180],
                })
    return findings
...
def audit_skill(path: Path) -> dict[str, object]:
    ...
    scripts = (
        sorted(str(p.relative_to(root)) for p in (root / "scripts").rglob("*") if p.is_file())
    ...
    if high or medium or scripts or hooks_present or allowed_tools_present:
