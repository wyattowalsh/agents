#!/usr/bin/env python3
"""Manage false-positive dismissals for the review learning loop.

Stores learnings in the active agent's reviews state directory.
Supports add, check, list, and clear subcommands. All output is JSON to stdout.

Usage:
  python learnings-store.py add --project my-app --finding-id RV-S-005 \
  ...
  cat findings.json | python learnings-store.py check --project my-app --record-match
...
LEARNINGS_DIR = get_agent_dir("reviews") / "learnings"
...
def matches_learning(finding: dict[str, Any], learning: dict[str, Any]) -> bool:
    ...
    Requires the same file and a relevant content match. Display ID prefixes are
    intentionally ignored because review IDs are frequently reassigned.
    """
    # --- File path check ---
    ...
    if finding_p != learning_p:
        return False

    # --- Content match (at least one must be true) ---

    # 1. Same exact finding/source ID.
    f_id = finding.get("id", "")
    f_source_id = finding.get("source_id", "")
    l_fid = learning.get("finding_id", "")
    if l_fid and l_fid in {f_id, f_source_id}:
        return True

    # 2. Same category plus meaningful keyword overlap.
    f_cat = (finding.get("category") or "").lower()
    l_cat = (learning.get("category") or "").lower()
    f_desc = finding.get("description", "") or finding.get("finding", "")
    l_pattern = learning.get("pattern", "")
    overlap = extract_keywords(f_desc) & extract_keywords(l_pattern)
    if f_cat and l_cat and f_cat == l_cat and len(overlap) >= 1:
        return True

    # 3. Strong keyword overlap when no category was stored.
...
def cmd_check(args: argparse.Namespace) -> None:
    ...
    if modified and args.record_match and not args.dry_run:
        save_learnings(args.project, learnings)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Manage false-positive dismissals for review.",
    ...
    sp_add.add_argument("--finding-id", required=True, help="Finding ID (e.g., RV-S-005).")
    ...
    sp_check.add_argument(
        "--record-match",
        action="store_true",
        help="Persist match-count updates for suppressions. Default check mode is read-only.",
    )
    sp_check.add_argument(
        "--dry-run",
        action="store_true",
        help="Compatibility no-op; check mode is read-only unless --record-match is set.",
    )
