...
def find_nonportable_body_operator_lines(body: str) -> list[dict]:
    """Return repo-root skills/*/scripts/ mentions outside fenced code blocks."""
    issues: list[dict] = []
    in_code_block = False
    for line_no, line in enumerate(body.splitlines(), 1):
        ...
        if in_code_block:
            continue
        for match in BODY_OPERATOR_SKILLS_PATH_RE.finditer(line):
            issues.append({
                "line": line_no,
                "match": match.group(0),
                "text": line.strip(),
...
def format_body_operator_issues(issues: list[dict], limit: int = 5) -> str:
    if not issues:
        return "No non-portable body operator paths found"
    rendered = [f"line {i['line']}: `{i['match']}`" for i in issues[:limit]]
    if len(issues) > limit:
        rendered.append(f"{len(issues) - limit} more")
    return "; ".join(rendered)
