...
## General Tips Across All Workflows

### Seed Management
- Always note the seed from the CLI output — it's your only way to reproduce a result.
- Lock seed (`--seed <value>`) when making incremental changes so you isolate the effect of each adjustment.
- Unlock (`--seed -1`) only when you want divergent variations.

### Strength (img2img / inpaint) Quick Reference

| `--strength` | Effect |
|-------------|--------|
| 0.2–0.3 | Near-identical to input; fine detail enhancement |
| 0.35–0.5 | Light modification; composition preserved |
| 0.6–0.75 | Significant change; main structure preserved |
| 0.85–1.0 | Near-complete redraw; only loose reference to input |

### Parameter Change Strategy
When iterating, **change one thing at a time**:
1. Prompt wording → controls content and composition
2. `--guidance-scale` → higher = literal, lower = creative
3. `--strength` → how much to change the input image
4. `--steps` → more detail (diminishing returns past 30–40)
5. Sampler → different aesthetic character (DPM++ 2M Karras vs Euler a)

### Output Organization
```bash
mkdir -p ~/Pictures/draw-thing/project-name
# Move outputs to project folder:
mv ~/Pictures/draw-thing/*.png ~/Pictures/draw-thing/project-name/
```
...
- `references/prompt-patterns.md` — model-specific prompt construction
- `references/controlnet-guide.md` — ControlNet weights, scheduling, multi-control
- `references/cli-reference.md` — complete flag reference
- `references/model-catalog.md` — which model to use for which task
