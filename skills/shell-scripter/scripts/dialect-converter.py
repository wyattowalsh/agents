...
CONVERSIONS = [
    ...
    ("bash", "zsh", r"#!/usr/bin/env bash", "#!/usr/bin/env zsh", "Update shebang to target dialect"),
    ("bash", "zsh", r"#!/bin/bash", "#!/usr/bin/env zsh", "Update shebang to target dialect"),
    # bash -> fish
    ("bash", "fish", r"#!/usr/bin/env bash", "#!/usr/bin/env fish", "Update shebang to target dialect"),
    ("bash", "fish", r"#!/bin/bash", "#!/usr/bin/env fish", "Update shebang to target dialect"),
    (
        "bash",
        "fish",
        r"^set -euo pipefail$",
        "# fish has built-in error handling",
        "fish does not use set -e; errors propagate naturally",
    ),
    ("bash", "fish", r"\bexport\s+(\w+)=(.*)", r"set -gx \1 \2", "fish uses set -gx instead of export"),
    ("bash", "fish", r"\$\{(\w+):-([^}]*)\}", r"set -q \1; or set \1 \2", "fish uses set -q for default values"),
    (
        "bash",
        "fish",
        r"\bif\s+\[\s+(.*?)\s+\];\s*then",
    ...
    ("bash", "fish", r"^\s*fi$", "end", "fish uses end instead of fi"),
    ("bash", "fish", r"^\s*done$", "end", "fish uses end instead of done"),
    ("bash", "fish", r"for\s+(\w+)\s+in\s+(.*?);\s*do", r"for \1 in \2", "fish for loops do not need do/done"),
    ("bash", "fish", r"\$\((.+?)\)", r"(\1)", "fish uses (cmd) instead of $(cmd)"),
    ("bash", "fish", r"\bfunction\s+(\w+)\s*\(\)\s*\{", r"function \1", "fish function syntax differs from bash"),
    ("bash", "fish", r"^(\w+)\s*\(\)\s*\{", r"function \1", "fish uses function keyword"),
    ("bash", "fish", r"^\s*\}$", "end", "fish uses end instead of }"),
    ("bash", "fish", r'\becho\s+(-[ne]\s+)?"\$\{(\w+)\[@\]\}"', r"echo $\2", "fish arrays are simpler"),
    ("bash", "fish", r"\[\[\s+(.*?)\s+\]\]", r"test \1", "fish uses test or builtin test"),
    # fish -> bash
    ("fish", "bash", r"#!/usr/bin/env fish", "#!/usr/bin/env bash", "Update shebang to target dialect"),
    ("fish", "bash", r"set -gx\s+(\w+)\s+(.*)", r"export \1=\2", "bash uses export instead of set -gx"),
    ("fish", "bash", r"set\s+(\w+)\s+(.*)", r"\1=\2", "bash uses var=value instead of set var value"),
    ("fish", "bash", r"if test\s+(.*)", r"if [ \1 ]; then", "bash uses [ ] with then"),
    ("fish", "bash", r"^\s*end$", "}", "bash uses } or fi/done instead of end"),
    ("fish", "bash", r"\(([^)]+)\)", r"$(\1)", "bash uses $(cmd) for command substitution"),
    ("fish", "bash", r"function\s+(\w+)$", r"\1() {", "bash function syntax"),
    # zsh -> bash
    ("zsh", "bash", r"#!/usr/bin/env zsh", "#!/usr/bin/env bash", "Update shebang to target dialect"),
    ("zsh", "bash", r"#!/bin/zsh", "#!/usr/bin/env bash", "Update shebang to target dialect"),
    ("zsh", "bash", r"\bsetopt\b.*", "# setopt removed (zsh-specific)", "setopt is zsh-specific"),
    ("zsh", "bash", r"\bunsetopt\b.*", "# unsetopt removed (zsh-specific)", "unsetopt is zsh-specific"),
    ("zsh", "bash", r"\bautoload\b.*", "# autoload removed (zsh-specific)", "autoload is zsh-specific"),
    # bash -> zsh additional
    ("bash", "zsh", r"\bdeclare\s+-a\s+", "typeset -a ", "zsh prefers typeset over declare"),
    ("bash", "zsh", r"\bdeclare\s+-A\s+", "typeset -A ", "zsh prefers typeset over declare"),
]
...
def main():
    ...
    parser.add_argument("--list-features", metavar="DIALECT", help="List features for a dialect")
