...
from nerdbot.operations import load_operation_entries as load_operation_entries
...
def classify_replay_status(
    ...
    failed: tuple[str, ...],
    *,
    operation_state: str = "committed",
) -> str:
    """Classify a dry-run replay result from path buckets."""
    if failed or operation_state == "failed":
        return "failed"
    if review_needed or operation_state in {"prepared", "review-needed"}:
...
def dry_run_replay_entry(entry: OperationEntry, existing_paths: set[str]) -> ReplayResult:
    ...
    return dry_run_replay(
        entry.operation_id,
        list(entry.changed_paths),
        existing_paths,
        operation_state=entry.status,
    )
