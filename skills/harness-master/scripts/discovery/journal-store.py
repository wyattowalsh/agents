#!/usr/bin/env python3
"""CRUD operations for discovery session journals in ~/.agents/harness-master/discovery/.
...
Default root: ~/.agents/harness-master/discovery/
Legacy read fallbacks (list/resume only): ~/.claude/discover-skills/ and related.
...
JOURNAL_DIR = get_agent_dir("harness-master/discovery")
ARCHIVE_DIR = JOURNAL_DIR / "archive"
SESSION_VERSION = 2

# Legacy locations (pre-consolidation discover-skills and intermediate) for read-only
# fallback in list/resume commands only. Writes always target JOURNAL_DIR.
LEGACY_JOURNAL_DIRS: list[Path] = [
    Path.home() / ".claude" / "discover-skills",
    Path.home() / ".claude" / "harness-master" / "discovery",
    Path.home() / ".agents" / "discover-skills",
]
...
def _resolve_path(target: str) -> Path | None:
    ...
    if result is not None:
        ...
        if not result.is_relative_to(jail):
            print(
                f"Error: path outside harness-master/discovery directory: {target}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _resolve_path_from_legacies(target: str) -> Path | None:
    """Resolve a journal target from legacy dirs only (for list/resume read fallback).
    Does not enforce JOURNAL_DIR containment; returns first match found.
    """
    for base in LEGACY_JOURNAL_DIRS:
        if not base.exists():
            continue
        # Direct path relative to this legacy base
        p = Path(target)
        if not p.is_absolute():
            p = base / p
        if p.resolve().is_file():
        ...
        # Try as number within this legacy base's list
        try:
            num = int(target)
            journals = list_journals(base)
            if 1 <= num <= len(journals):
                return Path(journals[num - 1]["path"]).resolve()
        except ValueError:
            pass

        # Try as keyword search within this legacy
        journals = list_journals(base)
        target_lower = target.lower()
        for j in journals:
            if (
...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    Includes legacy read fallback dirs (for pre-move journals) in addition to primary.
    """
    journals = list_journals(JOURNAL_DIR, args.filter)

    # legacy read fallback (list only)
    seen = {j.get("filename") for j in journals if j.get("filename")}
    for leg in LEGACY_JOURNAL_DIRS:
        for j in list_journals(leg, args.filter):
            fname = j.get("filename")
            if fname and fname not in seen:
                journals.append(j)
                seen.add(fname)

    # Re-sort reverse chrono by filename (YYYY-MM-DD- prefix)
    journals.sort(key=lambda j: j.get("filename", ""), reverse=True)
...
def cmd_resume(args: argparse.Namespace) -> None:
    ...
    Uses legacy read fallback for locating journals from ~/.claude/discover-skills etc.
    """
    if args.target:
        path = _resolve_path(args.target)
        if path is None:
            # legacy read fallback for resume-by-target (list/resume only)
            path = _resolve_path_from_legacies(args.target)
        if path is None:
            print(f"Error: journal not found: {args.target}", file=sys.stderr)
            sys.exit(1)
    else:
        # Find most recent in-progress journal (primary + legacy read fallback)
        journals = list_journals(JOURNAL_DIR, "active")
        if not journals:
            seen = {j.get("filename") for j in journals if j.get("filename")}
            for leg in LEGACY_JOURNAL_DIRS:
                for j in list_journals(leg, "active"):
                    fname = j.get("filename")
                    if fname and fname not in seen:
                        journals.append(j)
                        seen.add(fname)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="CRUD operations for discovery journals in ~/.agents/harness-master/discovery/.",
    )
