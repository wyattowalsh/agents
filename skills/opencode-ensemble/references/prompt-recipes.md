...
In this repo, omit `team_spawn.model` by default so teammates inherit OpenCode's configured agent variants.
...
Use before parallel builder waves.

```text
Map wave <N> for <goal>. Report:
1. Independent implementation lanes that can run in parallel
2. Shared files or contracts that must be serialized first
3. Required task dependencies for the next wave
4. Verification commands each lane should run
5. Risks that should use plan_approval
...
Use for one lane in a parallel builder wave.

```text
Implement wave <N> lane <lane name>: <specific behavior>.

Boundaries:
- Own only <files, subsystem, or vertical slice>.
- Do not edit shared contracts unless this task explicitly owns them.
- If another lane blocks you, message the lead instead of broadening scope.

Run <verification command> and report files changed, tests run, and residual risk.
```
...
team_spawn({
  name: "wave1-api",
  agent: "build",
  claim_task: "task_real_id",
  prompt: "<wave builder prompt>",
});
```

## Wave QA Prompt

Use after implementation branches have been merged into the lead worktree.

```text
Verify integrated wave <N> for <goal>. Prefer public API or user-flow tests.

Check:
- The merged behavior works across lanes
- Regressions caused by lane integration
- Missing boundary/error cases
- Unrelated files in the merged diff

Do not assume individual builder tests are sufficient. Run <verification command> and report findings.
```
