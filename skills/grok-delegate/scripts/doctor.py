#!/usr/bin/env python3
"""Portable Grok Build preflight doctor for grok-delegate (bundled, self-contained)."""
...
from auth_lib import collect_auth_checks
...
def collect_checks(*, cwd: Path, home: Path | None = None) -> list[dict[str, str]]:
    ...
    home_config = home / ".grok" / "config.toml"
    ...
    checks.extend(collect_auth_checks(home=home))
            ...
            if result.returncode == 0:
                ...
                checks.append(
                    _make_check(
                        ...
                        "Run grok login or verify OAuth tokens.",
                    )
                )
