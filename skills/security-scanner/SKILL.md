...
description: >-
  ...
  code review (review) or pen testing.
...
**Scope:** Pre-deployment security audit only. NOT for code review (use `review`),
authorized live offensive testing (use `pentest` with signed ROE), penetration
...
When `$ARGUMENTS` is empty, show the dispatch table above plus examples:
...
State that a full scan runs only after the user picks a mode (for example `scan`); empty invocation is menu-only.
