...
Run `uv run python scripts/complexity-scanner.py <path>` to get complexity metrics.
Parse JSON output. Flag functions with cyclomatic_complexity > 10 as HIGH, > 5 as MEDIUM.

### Scan Step 2: Dead Code Detection

Run `uv run python scripts/dead-code-detector.py <path>` to find unused code.
...
Run `uv run python scripts/dependency-staleness-checker.py <path>` to check outdated packages.
Parse JSON output. Deprecated packages are CRITICAL. Staleness > 365 days is HIGH.

### Scan Step 4: Pattern Consistency

Run `uv run python scripts/pattern-consistency-checker.py <path>` to detect inconsistencies.
