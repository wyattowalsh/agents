...
def _is_wagents_managed_or_mcphub_only(spec: dict[str, Any]) -> bool:
    tags = spec.get("tags") or []
    if isinstance(tags, str):
        tags = [tags]
    tagset = {str(t).lower() for t in tags}
    if "wagents_managed" in tagset or "mcphub-only" in tagset or "mcphub_only" in tagset:
        return True
    # Also treat servers that declare heavy mcphub ownership as managed unless they opt in
    ownership = spec.get("ownership") or {}
    # If only repo_mcp and no plugin/extension exposure, consider internal for APM export
    return bool(ownership and set(ownership.keys()) <= {"repo_mcp"})


def _is_repo_local_mcp(spec: dict[str, Any]) -> bool:
    cmd = str(spec.get("command", "") or "")
    args = " ".join(str(a) for a in (spec.get("args") or []))
    hay = f"{cmd} {args}"
    return "${REPO_ROOT}" in hay or hay.startswith("scripts/") or "/scripts/mcphub/" in hay or "mcphub" in hay.lower()


def materialize_mcp_fragment(repo_root: Path) -> list[dict[str, Any]]:
    """Return list suitable for dependencies.mcp in apm.yml.

    Only servers not tagged wagents_managed / mcphub-only.
    Repo-local (stdio with repo paths) emitted with registry:false .
    """
    reg_path = repo_root / "config" / "mcp-registry.json"
    if not reg_path.exists():
        return []
    try:
        registry = json.loads(reg_path.read_text(encoding="utf-8"))
    ...
    servers: dict[str, Any] = registry.get("servers", {})
    ...
    for name, spec in sorted(servers.items()):
        ...
        if not spec.get("enabled", True):
            continue
        if _is_wagents_managed_or_mcphub_only(spec):
            continue
        if _is_repo_local_mcp(spec):
            entry: dict[str, Any] = {
                "name": name,
            ...
            # Public/ npx etc -- declare by name (APM can resolve) or minimal inline
            # Use simple string form when possible; object for transport clarity
            fragment.append({"name": name, "transport": spec.get("transport", "stdio")})
    return fragment
