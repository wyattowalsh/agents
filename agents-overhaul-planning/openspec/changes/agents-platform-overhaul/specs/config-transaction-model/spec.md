# Spec Delta: config-transaction-model

## ADDED Requirements

### Requirement: Governed capability changes

Every stable capability introduced by the agents platform overhaul SHALL have a durable requirement in `openspec/specs/config-transaction-model/spec.md` or an active delta under `openspec/changes/agents-platform-overhaul/specs/config-transaction-model/spec.md`.
