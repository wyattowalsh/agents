...
- Catalog authoring rows: 1429
- Installable promoted curated-external rows: 1267
...
- Source-list evidence: 289 list-only source probes recorded; 1267 installable rows were promoted from reviewed override evidence.
...
- Status note: the recorded post-install dry-run covers 9 harnesses with 0 missing desired rows and 0 remaining commands.
...
- The promotion overlay records non-dry-run Skills CLI evidence; `harness-install-assurance.json` records the sanitized post-install reconciliation result, and `non-skill-install-assurance.json` records CLI, MCP, plugin, library, collection, and quarantine runtime dispositions.
...
- Required closeout: `uv run python scripts/apply_candidate_corpus_promotions.py --check` for 1267 promotion overrides.
- Required closeout: `uv run python scripts/audit_candidate_deep_sources.py --check` for 289 normalized targets.
- Required closeout: `uv run python scripts/promote_candidate_corpus.py --final-check` for 293 raw entries, 289 unique targets, 289 deep-audited targets, 0 deep terminal blocker, 1267 promoted overrides, and 1267 recorded install evidence rows.
