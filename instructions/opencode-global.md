...
Runtime-specific keys (`mode`, `temperature`, `color`, `permission`) for the built-in agents are stored in `config/opencode-agents.json` and projected into `.opencode/agents/*.md` by wagents sync. `instructions/opencode-agents-overlay.md` documents the overlay; it is not the machine SSOT. After `apm compile -t opencode` or `apm install`, run `just sync-opencode` then `just refresh-apm-lock` before commit (`apm run compile-opencode` chains both). The canonical agent definitions in `agents/*.md` contain only the portable schema (`name`, `description`, `tools`, `permissionMode`, etc.).
